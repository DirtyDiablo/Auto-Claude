...
def index_contacts(qdrant, conn, start_id: int, end_id: int, worker: int):
    ...
    total_in_range = conn.execute(
        "SELECT COUNT(*) FROM candidates WHERE id >= ? AND id <= ?",
        (start_id, end_id)
    ).fetchone()[0]
    ...
    for row in cursor:
        ...
        parts = [contact['name'], contact['title'], contact['company'],
                 contact['program'], contact['notes'], contact['city'], contact['state']]
        ...
        if len(batch_contacts) >= BATCH_SIZE:
            ...
            print(f"  [W{worker}] {indexed:,}/{total_in_range:,} ({100*indexed/total_in_range:.1f}%) | "
    ...
    if batch_contacts:
        try:
            embeddings = get_embeddings(batch_texts)
            points = [
                PointStruct(id=id_to_uuid(int(c['id']), NAMESPACE_CONTACTS), vector=e, payload=c)
                for c, e in zip(batch_contacts, embeddings)
            ]
            qdrant.upsert(collection_name=collection_name, points=points)
            indexed += len(points)
...
def index_activities(qdrant, conn, start_id: int, end_id: int, worker: int):
    ...
    # Count records in range
    total_in_range = conn.execute(
        "SELECT COUNT(*) FROM activities WHERE id >= ? AND id <= ?",
        (start_id, end_id)
    ).fetchone()[0]
    ...
    if batch_activities:
        try:
            embeddings = get_embeddings(batch_texts)
            points = [
                PointStruct(id=string_to_uuid(a['id'], NAMESPACE_ACTIVITIES), vector=e, payload=a)
                for a, e in zip(batch_activities, embeddings)
            ]
            qdrant.upsert(collection_name=collection_name, points=points)
            indexed += len(points)
...
def main():
    ...
    parser.add_argument('--type', required=True, choices=['contacts', 'activities'],
    ...
    parser.add_argument('--worker', type=int, required=True, help='Worker number (1 or 2)')
    ...
    if args.type == 'contacts':
        indexed, errors = index_contacts(qdrant, conn, args.start_id, args.end_id, args.worker)
    else:
        indexed, errors = index_activities(qdrant, conn, args.start_id, args.end_id, args.worker)
