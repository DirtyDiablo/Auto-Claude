...
class RegressionDetector:
        ...
        if self._storage_path:
            try:
                ...
                if os.path.exists(path):
                    with open(path) as f:
                        data = json.load(f)
                    self._regressions = [Regression(**r) for r in data.get("regressions", [])]
    ...
    async def check_regressions(self, current_metrics: Dict[str, float] = None) -> List[Regression]:
    ...
    async def auto_rollback(self, regression: Regression, optimizer=None) -> RollbackResult:
        if regression.caused_by and regression.caused_by != "unknown" and optimizer:
            ...
            if success:
                ...
                return RollbackResult(
                    regression_id=regression.regression_id, success=True,
                    message=f"Rolled back optimization {regression.caused_by}",
                    rolled_back_optimization=regression.caused_by,
                )

        return RollbackResult(
            regression_id=regression.regression_id, success=False,
    ...
    async def configure_thresholds(self, metric: str, warning: float = 0.10,
                                    critical: float = 0.25):
