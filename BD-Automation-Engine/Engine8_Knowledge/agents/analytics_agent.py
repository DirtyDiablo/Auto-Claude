...
class AnalyticsAgent(BDAgent):
    ...
    def analyze_hiring_trends(self, jobs: List[Dict]) -> List[TrendInsight]:
        ...
        if top_companies:
            insights.append(
                TrendInsight(
                    ...
                    confidence=0.9,
                    data_points=[
                        {"company": c[0], "count": c[1]} for c in top_companies
                    ],
                    actionable=True,
                )
            )
        ...
        ts_sci_count = sum(
            c for cl, c in clearance_counts.items() if cl and "ts/sci" in cl.lower()
        )
                    ...
                    data_points=[
                        {
                            "clearance": "TS/SCI",
                            "count": ts_sci_count,
                            "percentage": ts_sci_count / len(jobs),
                        }
                    ],
                    actionable=True,
                )
        ...
        if top_locations:
            insights.append(
                TrendInsight(
                    ...
                    confidence=0.85,
                    data_points=[
                        {"location": l[0], "count": l[1]} for l in top_locations
                    ],
                    actionable=True,
                )
            )
    ...
    def analyze_program_activity(
        self, programs: List[Dict], jobs: List[Dict]
    ) -> List[TrendInsight]:
        ...
        if top_programs:
            insights.append(
                TrendInsight(
                    ...
                    confidence=0.85,
                    data_points=[
                        {"program": p[0], "job_count": p[1]} for p in top_programs
                    ],
                    actionable=True,
                )
            )
    ...
    def analyze_contacts_by_tier(self, contacts: List[Dict]) -> List[TrendInsight]:
        ...
        high_value = sum(
            c for t, c in tier_counts.items() if "Tier 1" in t or "Tier 2" in t
        )
    ...
    def generate_report(
        ...
        key_metrics = {
            ...
            "ts_sci_jobs": sum(
                1
                for j in jobs
                if "ts/sci" in str(j.get("detected_clearance", "")).lower()
            ),
            "critical_contacts": sum(
                1 for c in contacts if c.get("bd_priority") == "Critical"
            ),
        }
            ...
            if insight.actionable and insight.confidence >= 0.8:
                if insight.category == "hiring" and "Top Hiring" in insight.title:
                    recommendations.append(
                        f"Focus on top-hiring companies for teaming opportunities"
                    )
                    ...
                    recommendations.append(
                        f"Initiate capture planning for recompete programs"
                    )
                elif insight.category == "contacts" and "Critical" in insight.title:
                    recommendations.append(
                        f"Schedule outreach to Critical priority contacts"
                    )
    ...
    async def process(
        ...
        return AgentResponse(
            ...
            content=(
                ...
                + "\n".join(
                    f"- {i.title}: {i.description}" for i in report.insights[:5]
                )
                + f"\n\nRecommendations:\n"
    ...
    test_contacts = [
        {
            "first_name": "John",
            "last_name": "Smith",
            "hierarchy_tier": "Tier 1 - Executive",
