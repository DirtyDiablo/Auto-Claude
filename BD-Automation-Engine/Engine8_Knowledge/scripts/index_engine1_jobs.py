...
def main():
    ...
    if ENGINE1_DIR.exists():
        for json_file in ENGINE1_DIR.glob("*.json"):
            ...
            total += index_records(
                qdrant, records, "jobs", build_job_text, json_file.name
            )
        ...
        for csv_file in ENGINE1_DIR.glob("*.csv"):
            ...
            total += index_records(
                qdrant, records, "jobs", build_job_text, csv_file.name
            )
    ...
    if DASHBOARD_DIR.exists():
        ...
        if jobs_json.exists():
            ...
            total += index_records(
                qdrant, records, "jobs", build_job_text, "dashboard_jobs.json"
            )
        ...
        if jobs_enriched.exists():
            ...
            if data and isinstance(data[0], dict) and "data" in data[0]:
                ...
                records = data
            total += index_records(
                qdrant, records, "jobs", build_job_text, "dashboard_jobs_enriched.json"
            )
