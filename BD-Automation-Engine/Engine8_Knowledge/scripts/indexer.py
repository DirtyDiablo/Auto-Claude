...
BULLHORN_DB_PATH = (
    PROJECT_ROOT / "Engine7_BullhornETL" / "data" / "bullhorn_intelligence.db"
)
FEDERAL_PROGRAMS_CSV = (
    PROJECT_ROOT
    / "Engine2_ProgramMapping"
    / "data"
    / "Federal_Programs_Master_Enriched_v2.csv"
)
OUTPUTS_DIR = PROJECT_ROOT / "outputs"
ANALYTICAL_OUTPUTS_DIR = (
    PROJECT_ROOT / "data" / "from_n8n_builder" / "analytical_outputs"
)
    ...
    def load_past_performance(path: Path = None) -> List[Dict]:
        ...
        if isinstance(data, dict):
            ...
            for i, item in enumerate(data):
                prime = item.get(
                    "prime_contractor", item.get("company", f"Unknown_{i}")
                )
                doc = {
                    "id": f"pp_{i}",
                    "title": f"Past Performance: {prime}",
                    ...
                    reports.append(
                        {
                            ...
                            "classification": "unclassified",
                            "date": datetime.fromtimestamp(
                                humint_file.stat().st_mtime
                            ).strftime("%Y-%m-%d"),
                        }
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to load HUMINT report {humint_file.name}: {e}"
                    )
            ...
            if briefings_dir.exists():
                for brief_file in sorted(briefings_dir.glob("*.md")):
                    try:
                        ...
                        logger.warning(
                            f"Failed to load briefing {brief_file.name}: {e}"
                        )
...
class BDIndexer:
    ...
    def __init__(
        self, store: BDKnowledgeStore = None, enable_auto_tagging: bool = True
    ):
        """
    ...
    def index_intelligence_reports(self, reports: List[Dict] = None) -> IndexingResult:
        ...
        indexed, errors = 0, 0
        if reports:
            indexed, errors = self.store._index_data(
                "intelligence_reports", reports, batch_size=50
            )
        ...
        logger.info(
            f"Intelligence reports indexing complete: {indexed} indexed, {errors} errors"
        )
        return result
    ...
    def _print_summary(self):
        ...
        for r in self.results:
            ...
            print(
                f"  {r.collection:12} | {r.indexed:5} indexed | {r.errors:3} errors | {r.duration_seconds:.1f}s | {status}"
            )
        ...
        print("-" * 60)
        print(
            f"  {'TOTAL':12} | {total_indexed:5} indexed | {total_errors:3} errors | {total_duration:.1f}s"
        )
        print("=" * 60)
    ...
    def save_index_report(self, output_path: Path = None) -> Path:
        ...
        if output_path is None:
            output_path = (
                PROJECT_ROOT / "Engine8_Knowledge" / "data" / "index_report.json"
            )
...
def main():
    ...
    parser.add_argument(
        "--activities", action="store_true", help="Index activities only"
    )
    parser.add_argument(
        "--intelligence", action="store_true", help="Index intelligence reports only"
    )
    parser.add_argument(
        "--force-recreate", action="store_true", help="Recreate collections"
    )
