...
def build_contact_text(r: dict) -> str:
    parts = []
    for field in ['Name', 'name', 'First Name', 'firstName', 'Last Name', 'lastName',
                  'Title', 'title', 'Job Title', 'jobTitle', 'Company', 'company',
                  'companyName', 'Organization', 'Email', 'email', 'Phone', 'phone',
                  'LinkedIn', 'linkedIn', 'Program', 'program', 'Prime', 'prime',
                  'Location', 'location', 'City', 'State', 'Notes', 'notes',
                  'Tier', 'tier', 'Classification', 'Skills', 'Clearance', 'clearance',
...
def main():
    ...
    for dir in [ENGINE3_DIR, ENGINE3_DATA]:
        ...
        if master_json.exists():
            ...
            total += index_records(qdrant, records, "contacts", build_contact_text, "Master_All_Contacts.json")
            break
    ...
    for dir in [ENGINE3_DIR, ENGINE3_DATA]:
        ...
        if prime_dir.exists():
            for csv_file in prime_dir.glob("*.csv"):
                ...
                total += index_records(qdrant, records, "contacts", build_contact_text, csv_file.name)
    ...
    for dir in [ENGINE3_DIR, ENGINE3_DATA]:
        ...
        if enriched_dir.exists():
            for csv_file in enriched_dir.glob("*.csv"):
                records = load_csv(csv_file)
                total += index_records(qdrant, records, "contacts", build_contact_text, f"enriched_{csv_file.name}")

    # Root level contact CSVs
    contact_files = [
        "DCGS_Contacts.csv", "DCGS_ContactsAll.csv",
        "GDIT_Other_Contacts.csv", "GDIT Other ContactsAll.csv",
        "GDIT PTS Contacts.csv", "GDIT PTS Contacts All.csv",
        "GBSD Contact Chart Updated 9 4.csv",
        "Lockheed Contact.csv", "Lockheed ContactAll.csv",
        "Contact_Search_List_MASTER.csv", "Bullhorn_Contact_Search.csv",
    ...
    for fname in contact_files:
        for dir in [ENGINE3_DIR, ENGINE3_DATA]:
            ...
            if path.exists():
                ...
                total += index_records(qdrant, records, "contacts", build_contact_text, fname)
                break
