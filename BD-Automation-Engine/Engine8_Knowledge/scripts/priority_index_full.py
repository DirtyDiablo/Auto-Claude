...
def create_collection(client: QdrantClient, name: str):
    ...
    if name in collections:
        ...
        logger.info(f"Collection {name} exists with {info.points_count} points - will append")
    else:
    ...
    for i in range(0, len(valid_records), BATCH_SIZE):
        ...
        try:
            ...
            if len(points) >= UPLOAD_BATCH_SIZE:
                ...
                logger.info(f"  Uploaded {len(points)} points | Total indexed: {count:,}")
                points = []
...
def build_candidate_text(r: Dict) -> str:
    parts = []
    for field in ['firstName', 'lastName', 'name', 'title', 'occupation',
                  'companyName', 'email', 'phone', 'address', 'city', 'state',
                  'skills', 'certifications', 'notes', 'description']:
        val = r.get(field)
...
def build_activity_text(r: Dict) -> str:
    parts = []
    for field in ['type', 'action', 'notes', 'comments', 'description',
                  'subject', 'contactName', 'candidateName', 'jobTitle']:
        val = r.get(field)
        if val and isinstance(val, str) and val.strip():
            parts.append(f"{field}: {val}")
    return " | ".join(parts)
...
def build_call_note_text(r: Dict) -> str:
    parts = []
    for field in ['action', 'comments', 'note_text', 'notes', 'person_name',
...
def build_document_text(r: Dict) -> str:
    parts = []
    for field in ['title', 'name', 'description', 'content', 'notes',
                  'text', 'body', 'summary', 'program', 'prime', 'contract']:
        val = r.get(field)
        if val and isinstance(val, str) and val.strip():
            parts.append(val)
    return " | ".join(parts)
...
def build_job_text(r: Dict) -> str:
    parts = []
    for field in ['title', 'jobTitle', 'description', 'requirements', 'skills',
                  'location', 'company', 'client', 'program', 'clearance', 'notes']:
        val = r.get(field)
        if val and isinstance(val, str) and val.strip():
            parts.append(f"{field}: {val}")
    return " | ".join(parts)
...
def build_program_text(r: Dict) -> str:
    parts = []
    for field in ['name', 'program_name', 'acronym', 'description', 'prime',
                  'prime_contractor', 'agency', 'branch', 'location', 'notes']:
        val = r.get(field)
        if val and isinstance(val, str) and val.strip():
            parts.append(f"{field}: {val}")
    return " | ".join(parts)
...
def index_bullhorn_database(qdrant: QdrantClient):
    ...
    process_records_parallel(candidates, "contacts", build_candidate_text, qdrant, "bullhorn_candidates")

    # Activities -> activities
    logger.info("\n[2/6] Loading activities...")
    activities = load_sqlite_table(BULLHORN_DB, "activities")
    process_records_parallel(activities, "activities", build_activity_text, qdrant, "bullhorn_activities")

    # Call notes -> activities
    logger.info("\n[3/6] Loading call_notes...")
    call_notes = load_sqlite_table(BULLHORN_DB, "call_notes")
    process_records_parallel(call_notes, "activities", build_call_note_text, qdrant, "bullhorn_call_notes")

    # Contact scores -> contacts
    logger.info("\n[4/6] Loading contact_scores...")
    scores = load_sqlite_table(BULLHORN_DB, "contact_scores")
    process_records_parallel(scores, "contacts", build_candidate_text, qdrant, "bullhorn_contact_scores")
    ...
    process_records_parallel(placements + past_perf, "documents", build_document_text, qdrant, "bullhorn_documents")
...
def index_colton_scurry(qdrant: QdrantClient):
    ...
    for filename, (collection, text_builder) in csv_mappings.items():
        ...
        if csv_path.exists():
            ...
            if records:
                process_records_parallel(records, collection, text_builder, qdrant, f"colton_{filename}")
...
def index_outputs_folder(qdrant: QdrantClient):
    ...
    for json_file in OUTPUTS_DIR.rglob("*.json"):
        ...
        if records:
            ...
            process_records_parallel(records, collection, builder, qdrant, f"outputs_{json_file.name}")
    ...
    for csv_file in OUTPUTS_DIR.rglob("*.csv"):
        ...
        if records:
            process_records_parallel(records, "documents", build_document_text, qdrant, f"outputs_{csv_file.name}")
