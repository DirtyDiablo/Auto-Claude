...
DASHBOARD_SOURCES = {
    ...
    "contacts": {
        "file": "contacts_classified.json",
        "collection": "contacts",
        "key_field": "name",
        "nested_key": "by_tier",
    },
    "programs": {
        "file": "programs_enriched.json",
        "collection": "programs",
        "key_field": "name",
    },
    "past_performance": {
        "file": "past_performance.json",
        "collection": "documents",
        "key_field": "prime_contractor",
        "doc_type": "past_performance",
    },
    "call_notes": {
        "file": "call_notes_summary.json",
        "collection": "activities",
        "key_field": "contact_name",
    },
    "call_notes_contacts": {
        "file": "call_notes_contacts.json",
        "collection": "activities",
        "key_field": "name",
    },
    "call_notes_primes": {
        "file": "call_notes_primes.json",
        "collection": "documents",
        "key_field": "name",
        "doc_type": "prime_analysis",
    },
    "call_notes_programs": {
        "file": "call_notes_programs.json",
        "collection": "documents",
        "key_field": "name",
        "doc_type": "program_analysis",
    },
    "placements": {
    ...
    "contractors": {
        "file": "contractors_enriched.json",
        "collection": "documents",
        "key_field": "name",
        "doc_type": "contractor_profile",
    },
    "prime_org_chart": {
        "file": "prime_org_chart.json",
        "collection": "documents",
        "key_field": "name",
        "doc_type": "prime_org",
    },
    "contact_org_chart": {
        "file": "contact_org_chart.json",
        "collection": "contacts",
        "key_field": "name",
        "nested_key": "tiers",
    },
...
CSV_SOURCES = {
    "federal_programs_master": {
        "file": "Federal Programs MASTER V4.csv",
        "collection": "programs",
        "key_field": "Program Name",
    },
    "insight_jobs": {
        "file": "Insight Global Jobs - Program Mapped (Dec 2025).csv",
        "collection": "jobs",
        "key_field": "Job Title",
    },
    "bd_opportunities": {
        "file": "BD Opportunities.csv",
        "collection": "documents",
        "key_field": "Opportunity Name",
        "doc_type": "opportunity",
    },
    "contractors_db": {
        "file": "Contractors Database.csv",
        "collection": "documents",
        "key_field": "Company Name",
        "doc_type": "contractor",
    },
    "contract_vehicles": {
        "file": "Contract_Vehicles.csv",
        "collection": "documents",
        "key_field": "Vehicle Name",
        "doc_type": "contract_vehicle",
    },
}
...
DATA_SCRAPER_SOURCES = {
    ...
    "output_root": {
        "path": "data/output",
        "files": [
            "bd_master_target_list.csv",
            "bd_all_scored_targets.csv",
            "bd_priority_targets.csv",
            "bd_large_dod_contracts.csv",
            "bd_it_services_targets.csv",
            "bd_hot_hiring_programs.csv",
            "bd_competitor_contract_presence.csv",
            "Federal_Programs_Enriched.csv",
            "phase1_tango_contracts.csv",
            "phase2_competitor_awards.csv",
            "phase3_opportunities_tango.csv",
            "phase3_solicitations_only.csv",
            "phase3_recompete_opportunities.csv",
            "phase4_new_high_value_contracts.csv",
            "phase6_org_intelligence.csv",
        ],
        "collection": "documents",
        "key_field": "name",
        "doc_type": "bd_output",
    ...
    "input_programs": {
        "path": "data/input",
        "files": [
            "Federal_ProgramsAll.csv",
            "contact_search_list.csv",
            "bd_targets.csv",
        ],
        "collection": "documents",
        "key_field": "name",
...
def record_to_content(record: Dict, key_field: str = None) -> str:
    ...
    for k, v in record.items():
        if k not in priority_fields and k not in [
            "id",
            "_id",
            "_tier",
            "_path",
...
class FullBDIndexer:
    ...
    def _index_source(self, root: Path, source_name: str, config: Dict, prefix: str):
        ...
        if config.get("type") == "sqlite":
            if source_path.exists():
                self._index_sqlite(
                    source_path, config.get("tables", []), f"{prefix}_{source_name}"
                )
            return
        ...
        if "pattern" in config and source_path.exists():
            ...
            files = list(
                source_path.rglob(pattern) if recursive else source_path.glob(pattern)
            )
            files = [f for f in files if f.is_file() and not should_skip_file(f)][
                :max_files
            ]
            ...
            if suffix == ".json":
                ...
                if data:
                    ...
                    if records:
                        ...
                        if size_mb > 1:
                            logger.info(
                                f"    ✓ {file_path.name} ({size_mb:.1f}MB): {indexed} records"
                            )
                ...
                if chunk_large and size_mb > 10:
                    ...
                    if total_indexed:
                        logger.info(
                            f"    ✓ {file_path.name} ({size_mb:.1f}MB): {total_indexed} records (chunked)"
                        )
                    self.stats["files_processed"] += 1
                else:
                    records = load_csv_full(file_path, MAX_RECORDS_PER_FILE)
    ...
    def _index_sqlite(self, db_path: Path, tables: List[str], source: str):
        ...
        try:
            ...
            tables_to_index = (
                [t for t in tables if t in available_tables]
                if tables
                else list(available_tables)
            )

            for table in tables_to_index:
                try:
                    cursor.execute(
                        f"SELECT * FROM {table} LIMIT {MAX_RECORDS_PER_FILE}"
                    )
                    ...
                    if records:
                        collection = (
                            "documents"
                            if table not in ["contacts", "jobs", "activities"]
                            else table
                        )
                        indexed = self._index_records(
                            records,
    ...
    def print_summary(self):
        ...
        total = sum(
            ...
            if k not in ["errors", "files_processed", "files_skipped"]
        )
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Index ALL data from all projects"
    )
    parser.add_argument(
        "--dashboard", action="store_true", help="Index dashboard JSON only"
    )
    parser.add_argument("--csv", action="store_true", help="Index CSV files only")
    parser.add_argument(
        "--external", action="store_true", help="Index external projects only"
    )
    parser.add_argument(
        "--data-scraper", action="store_true", help="Index Data-Scraper only"
    )
    parser.add_argument("--n8n", action="store_true", help="Index N8N Builder only")
    parser.add_argument(
        "--force", action="store_true", help="Force recreate collections"
    )
    parser.add_argument(
        "--lightrag", action="store_true", help="Also populate LightRAG"
    )
    ...
    parser.add_argument(
        "--collection",
        type=str,
        choices=["jobs", "contacts", "programs", "documents", "activities"],
        help="Index only this collection (for parallel processing)",
    )
    parser.add_argument(
        "--qdrant-path",
        type=str,
        help="Custom Qdrant storage path (for parallel instances)",
    )
    parser.add_argument(
        "--qdrant-url",
        type=str,
    ...
    indexer = FullBDIndexer(
        store=store, dry_run=args.dry_run, collection_filter=args.collection
    )
    ...
    if args.all or not any(
        [args.dashboard, args.csv, args.external, args.data_scraper, args.n8n]
    ):
    ...
    if args.lightrag and not args.dry_run:
        ...
        print("=" * 60)
        try:
            from Engine8_Knowledge.scripts.populate_lightrag import (
                main as populate_lightrag,
            )
