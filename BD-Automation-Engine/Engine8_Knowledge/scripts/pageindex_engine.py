...
class PageIndexEngine:
    ...
    def __init__(self, index_dir: str = None):
        self.index_dir = index_dir or os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "data", "pageindex"
        )
    ...
    def query(self, query: str, doc_ids: List[str] = None) -> RetrievalPath:
        ...
        search_trees = {
            did: self.trees[did]
            for did in (doc_ids or self.trees.keys())
        ...
        return RetrievalPath(
            query=query, path=path[:5], final_answer=answer, confidence=score
        )
