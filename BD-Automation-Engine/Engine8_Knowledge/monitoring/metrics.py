    ...
    def _initialize(self):
        try:
            ...
            self._request_counter = Counter(
                "pts_api_requests_total",
                "Total API requests",
                ["method", "endpoint", "status_code"],
            )
            self._request_duration = Histogram(
                "pts_api_request_duration_seconds",
                "Request duration",
                ["method", "endpoint"],
                buckets=[0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0],
            )
            self._request_in_progress = Gauge(
                "pts_api_requests_in_progress", "Requests in progress", ["endpoint"]
            )
            # Business metrics
            ...
            self._scrape_jobs = Counter(
                "pts_scrape_jobs_total", "Scrape jobs", ["source"]
            )
            self._search_queries = Counter(
                "pts_search_queries_total", "Search queries", ["type"]
            )
            self._ml_predictions = Counter(
                "pts_ml_predictions_total", "ML predictions", ["model"]
            )
            self._memory_ops = Counter(
                "pts_memory_operations_total", "Memory ops", ["layer", "operation"]
            )
            self._workflow_execs = Counter(
                "pts_workflow_executions_total",
                "Workflow executions",
                ["workflow", "status"],
            )
            # Infra metrics
            ...
            self._db_query_duration = Histogram(
                "pts_db_query_duration_seconds", "DB query duration", ["db_type"]
            )
