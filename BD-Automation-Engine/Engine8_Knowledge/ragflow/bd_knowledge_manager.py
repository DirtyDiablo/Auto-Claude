...
from .ragflow_client import RAGflowClient, ChunkMethod, QueryResult, get_ragflow_client
...
class BDKnowledgeManager:
    ...
    def _ensure_initialized(self) -> None:
        ...
        if not self._initialized:
            raise RuntimeError(
                "BDKnowledgeManager not initialized. Call initialize() first."
            )
    ...
    async def get_kb_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        for kb_name, kb_id in self.kb_ids.items():
            try:
                ...
                stats[kb_name] = {"kb_id": kb_id, "status": "error", "error": str(e)}
    ...
    async def ingest_project_files(
        self, project_path: str, auto_categorize: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        for file_path in all_files:
            ...
            try:
                doc_id = await self._client.upload_document(
                    kb_id, str(file_path), BD_KNOWLEDGE_BASES[target_kb].chunk_method
                )
                results[target_kb].append(
                    {"file": str(file_path), "doc_id": doc_id, "status": "uploaded"}
                )
            except Exception as e:
    ...
    async def upload_to_kb(self, kb_name: str, file_path: str) -> str:
        """Upload a single file to a specific KB."""
        self._ensure_initialized()

        if kb_name not in self.kb_ids:
            raise ValueError(
                f"Unknown KB: {kb_name}. Available: {list(self.kb_ids.keys())}"
            )
    ...
    async def query_bd_intelligence(
        self, question: str, context: str = "general", top_k: int = 5
    ) -> QueryResult:
        """
        ...
        return await self._client.query(
            kb_ids=kb_ids, question=question, top_k=top_k, with_answer=True
        )

    async def get_program_intelligence(
        self, program_name: str, include_related: bool = True
    ) -> ProgramIntelligence:
        """
        ...
        async def query_kb(kb_name: str, query: str) -> tuple:
            ...
            try:
                result = await self._client.query(
                    [kb_id], query, top_k=5, with_answer=False
                )
        ...
        results = await asyncio.gather(*[query_kb(kb, q) for kb, q in queries])
    ...
    async def get_contact_context(self, contact_name: str) -> ContactContext:
        """
    ...
    async def generate_call_prep(
        self, contact_name: str, program_context: Optional[str] = None
    ) -> CallPrepBrief:
        """
        ...
        result = await self._client.query(
            kb_ids=all_kb_ids, question=query, top_k=15, with_answer=True
        )
        ...
        brief = CallPrepBrief(
            ...
            contact_background=self._extract_section(
                answer, "background", "Contact background not found"
            ),
    ...
    def _extract_list(self, text: str, keyword: str) -> List[str]:
        ...
        return (
            items if items else [f"No {keyword}s identified - review source documents"]
        )
