...
def mock_hub():
    ...
    hub.post = AsyncMock(
        return_value={
            "results": {
                "episodic": [
                    {"content": "DCGS meeting notes", "score": 0.8, "id": "m1"}
                ],
                "procedural": [
                    {"content": "Cold email pattern", "score": 0.7, "id": "m2"}
                ],
            },
            "total_results": 2,
        }
...
class TestAddMemory:
    ...
    async def test_add_memory(self, tools, mock_hub):
        mock_hub.post = AsyncMock(
            return_value={"memory_id": "mem_new_123", "layer": "auto"}
        )
        fns, _ = tools
    ...
    async def test_add_with_contact(self, tools, mock_hub):
        mock_hub.post = AsyncMock(
            return_value={"memory_id": "mem_c_456", "layer": "auto"}
        )
        fns, _ = tools
