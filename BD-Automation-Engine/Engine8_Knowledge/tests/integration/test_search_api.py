...
def mock_unified():
    ...
    us.multi_search.return_value = [
        SearchResponse(
            results=[SearchResult(id="1", content="r1", score=0.8, source="bd_contacts")],
            mode_used="hybrid",
...
class TestSearchEndpoints:
    ...
    def test_unified_search(self, client, mock_unified):
        with patch("Engine8_Knowledge.search.search_routes.get_unified_search", return_value=mock_unified):
    ...
    def test_search_with_filters(self, client, mock_unified):
        with patch("Engine8_Knowledge.search.search_routes.get_unified_search", return_value=mock_unified):
            resp = client.post("/search/v2", json={
                "query": "DCGS",
...
class TestInfoEndpoints:
    ...
    def test_search_stats(self, client):
        with patch("Engine8_Knowledge.search.search_routes.RESULTS_DIR", Path("/nonexistent")):
...
class TestBenchmarkEndpoints:
    ...
    def test_run_benchmark(self, client):
        ...
        with patch("Engine8_Knowledge.search.search_routes.SearchBenchmarkV2", return_value=mock_bench):
    ...
    def test_latest_benchmark_exists(self, client):
        mock_bench = MagicMock()
        mock_bench.get_latest_report.return_value = {"timestamp": "2026-01-01", "results": {"hybrid": {}}}
        with patch("Engine8_Knowledge.search.search_routes.SearchBenchmarkV2", return_value=mock_bench):
            resp = client.get("/search/v2/benchmark/latest")
        assert resp.status_code == 200
