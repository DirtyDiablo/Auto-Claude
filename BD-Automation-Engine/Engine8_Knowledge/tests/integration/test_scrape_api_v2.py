...
def mock_sam_sync():
    ...
    sam.search_opportunities = AsyncMock(
        return_value=[
            ContractOpportunity(
                notice_id="OPP-001",
                title="DCGS RFI",
    ...
    sam.list_watches = MagicMock(
        return_value=[
            WatchConfig(watch_id="w1", name="DCGS Monitor", keywords=["DCGS"]),
        ]
    )
    sam.add_watch = MagicMock(
        return_value=WatchConfig(
            watch_id="w_new",
            name="ISR Watch",
            keywords=["ISR"],
...
def mock_doc_pipeline():
    doc = AsyncMock()
    doc.discover_documents = AsyncMock(
        return_value=[
            DocumentRef(
                doc_id="doc_001", url="https://sam.gov/doc.pdf", title="Test RFP"
            ),
        ]
    )
    doc.process_document = AsyncMock(
        return_value=ProcessedFederalDoc(
...
def mock_orchestrator():
    ...
    orch.list_sources = AsyncMock(
        return_value=[
            SourceStatus(
                config=SourceConfig(
                    source_id="s1", name="GDIT Careers", source_type="career_page"
                ),
                status="active",
                total_runs=5,
            ),
    ...
    orch.get_source_health = AsyncMock(
    ...
    orch.get_orchestrator_stats = AsyncMock(
        return_value=OrchestratorStats(
            total_sources=5,
            active_sources=3,
            paused_sources=1,
            errored_sources=1,
...
class TestCrawlEndpoints:
    def test_crawl_url(self, client, mock_crawl_engine):
        with patch(
            "Engine8_Knowledge.api_routers.scrape_api_v2._get_crawl_engine",
            return_value=mock_crawl_engine,
        ):
            resp = client.post(
                "/scrape/crawl",
                json={
...
class TestOrchestratorEndpoints:
    def test_trigger_full_cycle(self, client, mock_orchestrator):
        with patch(
            "Engine8_Knowledge.api_routers.scrape_api_v2._get_orchestrator",
    ...
    def test_trigger_source_not_found(self, client, mock_orchestrator):
        mock_orchestrator.trigger_source = AsyncMock(
            side_effect=ValueError("Source not found: bad_id")
        )
        with patch(
            "Engine8_Knowledge.api_routers.scrape_api_v2._get_orchestrator",
            return_value=mock_orchestrator,
...
class TestSAMEndpoints:
    def test_search_awards(self, client, mock_sam_sync):
        with patch(
            "Engine8_Knowledge.api_routers.scrape_api_v2._get_sam_sync",
            return_value=mock_sam_sync,
        ):
            resp = client.post(
                "/sam/search-awards",
                json={
...
class TestFederalDocEndpoints:
    def test_discover_docs(self, client, mock_doc_pipeline):
        with patch(
            "Engine8_Knowledge.api_routers.scrape_api_v2._get_doc_pipeline",
            return_value=mock_doc_pipeline,
        ):
            resp = client.post(
                "/federal-docs/discover",
                json={
                    "source": "sam_gov",
                    "keywords": ["DCGS"],
                },
    ...
    def test_process_doc_not_found(self, client, mock_doc_pipeline):
        mock_doc_pipeline.process_document = AsyncMock(
            side_effect=FileNotFoundError("File not found: /tmp/missing.pdf")
        )
        with patch(
            "Engine8_Knowledge.api_routers.scrape_api_v2._get_doc_pipeline",
            return_value=mock_doc_pipeline,
