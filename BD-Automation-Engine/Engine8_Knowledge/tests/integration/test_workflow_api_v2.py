...
from Engine8_Knowledge.workflows.orchestrator_v2 import WorkflowExecution, ScheduledWorkflow
from Engine8_Knowledge.workflows.time_travel import ExecutionTimeline, StateDiff, NodeVisit
...
MOCK_EXECUTION = WorkflowExecution(
    thread_id="t1", workflow_name="test_wf", status="completed",
    started_at="2024-01-01T00:00:00", step_count=3,
)

MOCK_INTERRUPTED = WorkflowExecution(
    thread_id="t2", workflow_name="test_wf", status="interrupted",
    started_at="2024-01-01T00:00:00", current_node="review", step_count=2,
    pending_approvals=["approval_123"],
)

MOCK_SCHEDULE = ScheduledWorkflow(
    schedule_id="sched_1", workflow_name="test_wf",
    cron_expression="0 6 * * *", default_input={}, enabled=True, run_count=5,
)

MOCK_APPROVAL = ApprovalRequest(
    request_id="approval_123", thread_id="t2",
    workflow_name="test_wf", node_name="review",
    description="Please approve", state_snapshot={},
    options=["approve", "reject"], urgency="high",
    created_at="2024-01-01T00:00:00", expires_at="2024-01-02T00:00:00",
    status="pending",
)

MOCK_DECISION = ApprovalDecision(
    request_id="approval_123", decision="approve",
    modified_state=None, notes="OK", decided_by="user",
    decided_at="2024-01-01T01:00:00",
)

MOCK_TIMELINE = ExecutionTimeline(
    thread_id="t1", workflow_name="test_wf",
    started_at="2024-01-01T00:00:00", completed_at="2024-01-01T00:01:00",
    status="completed", total_steps=3, total_duration_seconds=60.0,
    nodes_visited=[NodeVisit(step=1, node_name="a", started_at="2024-01-01T00:00:00",
                              duration_seconds=1.0, state_keys_modified=["x"],
                              input_summary="{}", output_summary="{x}")],
    errors=[], interrupts=[],
)

MOCK_DIFF = StateDiff(step_a=1, step_b=2, added_keys=["y"],
                       removed_keys=[], modified_keys={})

MOCK_WORKFLOW_INFO = WorkflowInfo(
    name="test_wf", description="Test", node_count=3, edge_count=2,
    interrupt_nodes=[], parallel_groups=[], entry_point="node_a",
)
...
def test_start_workflow():
    with patch("Engine8_Knowledge.workflows.workflow_routes._get_orchestrator", return_value=_mock_orch()):
...
def test_start_workflow_bad_name():
    ...
    with patch("Engine8_Knowledge.workflows.workflow_routes._get_orchestrator", return_value=mock):
...
def test_get_timeline():
    with patch("Engine8_Knowledge.workflows.workflow_routes._get_debugger", return_value=_mock_debugger()):
...
def test_create_schedule():
    with patch("Engine8_Knowledge.workflows.workflow_routes._get_orchestrator", return_value=_mock_orch()):
        resp = client.post("/workflows/schedules", json={
            "workflow_name": "test_wf", "cron_expression": "0 6 * * *",
        })
        assert resp.status_code == 200


def test_list_approvals():
    with patch("Engine8_Knowledge.workflows.workflow_routes._get_hitl", return_value=_mock_hitl()):
...
def test_submit_decision():
    with patch("Engine8_Knowledge.workflows.workflow_routes._get_hitl", return_value=_mock_hitl()):
        resp = client.post("/workflows/approvals/approval_123/decide",
