...
def mock_graph_queries():
    ...
    gq.find_shortest_path.return_value = {
        "from": "A",
        "to": "B",
        "path": [],
        "found": False,
    ...
    gq.get_program_org_chart.return_value = {
        "program": "test",
        "managers": [],
        "team": [],
    }
    gq.get_company_network.return_value = {
        "company": "test",
        "programs": [],
        "people": [],
        "subs": [],
    ...
    gq.find_competitive_overlap.return_value = {
        "company_a": "A",
        "company_b": "B",
        "overlap_programs": [],
        "overlap_count": 0,
    }
    gq.get_location_intel.return_value = {
        "location": "test",
        "programs": [],
        "people": [],
        "jobs": [],
...
class TestHealthEndpoints:
    ...
    def test_health(self, client, mock_neo4j_mgr):
        with patch(
            "Engine8_Knowledge.graph.neo4j_routes.get_neo4j_manager",
            return_value=mock_neo4j_mgr,
        ):
    ...
    def test_stats(self, client, mock_graph_queries):
        with patch(
            "Engine8_Knowledge.graph.neo4j_routes.get_graph_queries",
            return_value=mock_graph_queries,
        ):
    ...
    def test_schema(self, client, mock_neo4j_mgr):
        with (
            ...
            patch(
                "Engine8_Knowledge.graph.neo4j_routes.get_schema_info",
                return_value={"node_types": [], "constraints": []},
            ),
        ):
    ...
    def test_apply_schema(self, client, mock_neo4j_mgr):
        with (
            ...
            patch(
                "Engine8_Knowledge.graph.neo4j_routes.apply_schema",
                return_value={"constraints": [], "indexes": [], "errors": []},
            ),
        ):
...
class TestIngestionEndpoints:
    ...
    def _mock_engine(self):
        ...
        engine.ingest_interactions.return_value = {
            "status": "completed",
            "ingested": 200,
        }
    ...
    def test_ingest_contacts(self, client, mock_neo4j_mgr):
        ...
        with (
            ...
            patch(
                "Engine8_Knowledge.graph.neo4j_routes.GraphIngestionEngine",
                return_value=engine,
            ),
        ):
