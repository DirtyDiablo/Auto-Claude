...
PRIME_PATTERNS = {
    'CACI': [r'\bCACI\b'],
    'GDIT': [r'\bGDIT\b', r'\bGeneral Dynamics IT\b', r'\bGeneral Dynamics Information Technology\b'],
    'Leidos': [r'\bLeidos\b'],
    'Lockheed Martin': [r'\bLockheed\b', r'\bLMCO\b', r'\bLockheed Martin\b', r'\bLM\s'],
    'SAIC': [r'\bSAIC\b'],
...
class BullhornParser:
    ...
    def build_prime_databases(self):
        ...
        for prime, contacts in sorted(prime_contacts.items(), key=lambda x: -len(x[1])):
            ...
            for c in contacts:
                ...
                recent_note = ''
                if c['notes']:
                    sorted_notes = sorted(c['notes'], key=lambda x: x['date'], reverse=True)
                ...
                rows.append({
                    ...
                    'Status': ', '.join(sorted(c['statuses'])) if c['statuses'] else '',
    ...
    def build_master_database(self):
        ...
        for contact_key, contact_data in self.contacts.items():
            ...
            master_contacts.append({
                ...
                'last_activity': max(contact_data['dates']) if contact_data['dates'] else None,
    ...
    def generate_report(self, summary: List[Dict]):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write("## Summary\n\n")
            f.write(f"| Prime | Contacts | With Programs | With Clearances | Total Notes |\n")
            f.write(f"|-------|----------|---------------|-----------------|-------------|\n")

            for s in sorted(summary, key=lambda x: -x['contact_count']):
                f.write(f"| {s['prime']} | {s['contact_count']} | {s['with_programs']} | {s['with_clearances']} | {s['total_notes']} |\n")
            ...
            for s in summary:
                if s['contact_count'] >= 100:
                    f.write(f"- **{s['prime']}**: {s['contact_count']} contacts - Review and deduplicate\n")
            ...
            for s in summary:
                if s['contact_count'] < 20:
                    f.write(f"- **{s['prime']}**: Only {s['contact_count']} contacts - NEEDS ZOOMINFO\n")
