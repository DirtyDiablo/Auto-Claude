...
DASHBOARD_DATA_DIR = (
    Path(__file__).parent.parent.parent / "dashboard" / "public" / "data"
)
    ...
    for filename in files_to_load:
        ...
        if filepath.exists():
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                print(
                    f"  {filename}: {len(data[filename.replace('.json', '')])} records"
                )
        else:
...
def create_past_performance_dashboard_data(bullhorn_data, federal_programs):
    ...
    for pp in bullhorn_data["past_performance"]:
        ...
        record = {
            ...
            "first_job_date": str(pp.get("first_job_date", ""))[:10]
            if pp.get("first_job_date")
            else None,
            "last_job_date": str(pp.get("last_job_date", ""))[:10]
            if pp.get("last_job_date")
            else None,
            "estimated_annual_revenue": (pp.get("total_placements", 0) or 0)
            * (pp.get("avg_bill_rate", 0) or 0)
            * 2080,
            "relationship_strength": "Strategic"
            if (pp.get("total_placements", 0) or 0) >= 50
            else "Key Account"
            if (pp.get("total_placements", 0) or 0) >= 20
            else "Established"
            if (pp.get("total_placements", 0) or 0) >= 10
            else "Growing"
            if (pp.get("total_placements", 0) or 0) >= 5
            else "Emerging",
            "is_defense_prime": prime_name
            in [
                "Leidos",
                "General Dynamics IT",
                "Peraton",
...
def create_prime_org_chart(bullhorn_data, federal_programs):
    ...
    for prog in federal_programs:
        prime = prog.get("Prime Contractor", "") or prog.get(
    ...
    for prime in bullhorn_data["primes"]:
        ...
        contacts = [
            c
            for c in bullhorn_data["contact_scores"]
        ...
        record = {
            ...
            "top_contacts": [
                {"name": c["contact_name"], "score": c["score"], "tier": c["tier"]}
...
def create_contact_org_chart(bullhorn_data):
    ...
    print(
        f"  Contact Org Chart: {len(bullhorn_data['contact_scores'])} contacts across {len(tier_groups)} tiers"
    )
    return org_chart
...
def create_program_org_chart(bullhorn_data, federal_programs):
    ...
    for prog in federal_programs:
        ...
        record = {
            ...
            "contract_value": prog.get("Contract Value", "")
            or prog.get("Contract Value (Consolidated)", ""),
            ...
            "subcontractors": prog.get("Key Subcontractors", "")
            or prog.get("Known Subcontractors", ""),
        }
        org_chart.append(record)
...
def create_placements_dashboard_data(bullhorn_data):
    ...
    for plc in bullhorn_data["placements"]:
        record = {
            ...
            "owner": plc.get("owner", ""),
            "start_date": str(plc.get("start_date", ""))[:10]
            if plc.get("start_date")
            else None,
            "end_date": str(plc.get("end_date", ""))[:10]
            if plc.get("end_date")
            else None,
            ...
            "margin_percent": round(
                ((plc.get("bill_rate", 0) or 0) - (plc.get("pay_rate", 0) or 0))
                / (plc.get("bill_rate", 0) or 1)
                * 100,
                1,
            )
            if plc.get("bill_rate")
            else 0,
        }
...
def create_correlation_summary(bullhorn_data, federal_programs, existing_data):
    ...
    defense_revenue = sum(
        (p.get("bill_rate", 0) or 0) * 2080
        for p in bullhorn_data["placements"]
        if (p.get("client_name") or "") in defense_primes
        and p.get("bill_rate")
        and p.get("bill_rate") < 500
    )
    ...
    summary = {
        ...
        "data_sources": {
            ...
            "bullhorn": {
                ...
                "primes": total_primes,
                "activities": sum(
                    a.get("count", 0) for a in bullhorn_data["activity_summary"]
                ),
            },
        ...
        "financial_metrics": {
            ...
            "defense_share_percent": round(defense_revenue / total_revenue * 100, 1)
            if total_revenue > 0
            else 0,
            "avg_bill_rate": round(
                sum(
                    p.get("bill_rate", 0) or 0
                    for p in bullhorn_data["placements"]
                    if p.get("bill_rate") and p.get("bill_rate") < 500
                )
                / len(
        ...
        "contact_metrics": {
            ...
            "avg_score": round(
                ...
                / len(bullhorn_data["contact_scores"])
                if bullhorn_data["contact_scores"]
                else 0,
                1,
            ),
        },
        "program_metrics": {
            ...
            "programs_with_placements": len(
                set(l.get("program_name") for l in bullhorn_data["program_links"])
            ),
        ...
        "prime_metrics": {
            ...
            "defense_primes": len(
                [
                    p
                    for p in bullhorn_data["primes"]
                    if p.get("category") == "Defense Prime"
                ]
            ),
            "top_primes": [
                {"name": p.get("name"), "placements": p.get("total_placements", 0)}
                for p in sorted(
                    bullhorn_data["primes"],
                    key=lambda x: x.get("total_placements", 0) or 0,
                    reverse=True,
                )[:10]
            ],
        },
...
def save_dashboard_files(data_dict):
    ...
    for filename, data in data_dict.items():
        ...
        record_count = (
            ...
            else sum(len(v.get("contacts", [])) for v in data.get("tiers", {}).values())
            if "tiers" in data
        ...
        print(
            f"  Saved: {filepath.name} ({size_bytes // 1024}KB, {record_count} records)"
        )
...
def run_integration():
    ...
    # Past Performance
    dashboard_files["past_performance"] = create_past_performance_dashboard_data(
        bullhorn_data, federal_programs
    )

    # Prime Org Chart
    dashboard_files["prime_org_chart"] = create_prime_org_chart(
        bullhorn_data, federal_programs
    )
    ...
    # Program Org Chart
    dashboard_files["program_org_chart"] = create_program_org_chart(
        bullhorn_data, federal_programs
    )
    ...
    dashboard_files["correlation_summary_enriched"] = create_correlation_summary(
        bullhorn_data, federal_programs, existing_data
    )
