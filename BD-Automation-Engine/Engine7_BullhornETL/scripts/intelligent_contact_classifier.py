...
class IntelligentContactClassifier:
    ...
    def _parse_notes_file(self, filepath: str) -> int:
        ...
        for _, row in df.iterrows():
            ...
            if note_date and note_date != "nan":
                if (
                    not contact["first_activity"]
                    or note_date < contact["first_activity"]
                ):
    ...
    def _determine_contact_type(
        self, contact: dict, note: str, note_type: str, status: str
    ):
            ...
            rows.append(
                {
                    ...
                    "All Titles": ", ".join(sorted(c["job_titles"]))
                    if c["job_titles"]
                    else "",
                    ...
                    "Last Activity": c["last_activity"][:10]
                    if c["last_activity"]
                    else "",
                    "First Activity": c["first_activity"][:10]
                    if c["first_activity"]
                    else "",
                    "Email": c["email"],
                    "Phone": c["phone"],
                    ...
                    "Statuses": ", ".join(sorted(c["statuses"]))
                    if c["statuses"]
                    else "",
                    "Hiring Signals": " | ".join(recent_signals)
                    if recent_signals
                    else "",
                    "Pain Points": " | ".join(list(c["pain_points"])[:3])
                    if c["pain_points"]
                    else "",
    ...
    def generate_intelligence_report(self):
        ...
        clients = len(
            [c for c in self.contacts.values() if c["contact_type"] == "client"]
        )
        candidates = len(
            [c for c in self.contacts.values() if c["contact_type"] == "candidate"]
        )
        hiring_managers = len(
            [c for c in self.contacts.values() if c["is_hiring_manager"]]
        )
        with_reqs = len([c for c in self.contacts.values() if c["has_open_reqs"]])
        decision_makers = len(
            [c for c in self.contacts.values() if c["is_decision_maker"]]
        )
        ...
        for c in hiring_mgrs:
            ...
            report.append(
                f"| {c['name']} | {primes} | {c['relationship_score']} | {'Yes' if c['has_open_reqs'] else 'No'} | {c['last_activity'][:10] if c['last_activity'] else ''} |"
            )
