...
def insert_program_mentions(
    conn: sqlite3.Connection, programs_analysis: dict, gap_programs: list
):
...
def insert_contact_activity(
    conn: sqlite3.Connection, contacts: list, gap_contacts: list
):
...
def update_contact_scores(conn: sqlite3.Connection):
    ...
    for contact_name, data in activity_data.items():
        ...
        cursor.execute(
            "SELECT id, score FROM contact_scores WHERE contact_name = ?",
            (contact_name,),
        )
        ...
        if existing:
            ...
            activity_modifier = (
                data["engagement"] // 10
            )  # 0-10 points based on engagement
...
def generate_summary_stats(conn: sqlite3.Connection):
    ...
    # Gap contacts count
    cursor.execute(
        "SELECT COUNT(*) FROM contact_activity_summary WHERE is_gap_contact = 1"
    )
