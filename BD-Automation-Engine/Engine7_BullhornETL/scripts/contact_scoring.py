...
def calculate_contact_score(contact_data: dict) -> dict:
    ...
    if last_activity:
        try:
            ...
            if recency_score > 0:
                factors.append(
                    f"Recent activity ({days_ago} days ago): +{recency_score}"
                )
...
def run_contact_scoring():
    ...
    for row in placement_data:
        ...
        if name:
            ...
            if row["start_date"]:
                ...
                if (
                    contacts[name]["last_activity_date"] is None
                    or date_str > contacts[name]["last_activity_date"]
                ):
    ...
    for contact in scored_contacts[:25]:
        ...
        print(
            f"{contact['name'][:35]:<35} {contact['score']:<8} {contact['tier']:<15} {contact['placements']:<12} {primes_str[:20]:<20}"
        )
    ...
    report = {
        ...
        "summary": {
            ...
            "avg_score": sum(c["score"] for c in scored_contacts) / len(scored_contacts)
            if scored_contacts
            else 0,
        },
    ...
    report_path = (
        OUTPUT_DIR / f"contact_scores_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    # Export to CSV
    csv_path = (
        OUTPUT_DIR / f"contact_scores_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
