...
def analyze_prime(prime_name: str, df: pd.DataFrame) -> dict:
    ...
    analysis["top_contacts"] = sorted(
        analysis["top_contacts"], key=lambda x: x["note_count"], reverse=True
    )[:20]

    # Sort recent activity
    analysis["recent_activity"] = sorted(
        analysis["recent_activity"], key=lambda x: x["last_activity"], reverse=True
    )[:20]
    ...
    for loc in expected_locations:
        if analysis["locations"].get(loc, 0) < 5:
            analysis["gaps"].append(
                f"Location: {loc} ({analysis['locations'].get(loc, 0)} contacts)"
            )

    for prog in expected_programs:
        if analysis["programs"].get(prog, 0) < 3:
            analysis["gaps"].append(
                f"Program: {prog} ({analysis['programs'].get(prog, 0)} contacts)"
            )
    ...
    for title_type in ["Program Manager", "Staffing/TA", "Contracts"]:
        if len(analysis["key_titles"].get(title_type, [])) < 3:
            analysis["gaps"].append(
                f"Title: {title_type} (only {len(analysis['key_titles'].get(title_type, []))} found)"
            )
...
def generate_report(analyses: list) -> str:
    ...
    report.append(
        "| Prime | Total Contacts | With Programs | With Clearances | Key Title Contacts | Gaps |"
    )
    report.append(
        "|-------|----------------|---------------|-----------------|-------------------|------|"
    )
    ...
    for a in sorted(analyses, key=lambda x: -x["total_contacts"]):
        ...
        gaps = len(a["gaps"])
        report.append(
            f"| {a['prime']} | {a['total_contacts']} | {a['with_programs']} | {a['with_clearances']} | {key_titles} | {gaps} |"
        )
    ...
    for a in sorted(analyses, key=lambda x: -x["total_contacts"]):
        ...
        if a["top_contacts"]:
            ...
            for c in a["top_contacts"][:10]:
                report.append(
                    f"| {c['name']} | {c['note_count']} | {c['programs'][:30] if c['programs'] else ''} | {c['clearances'][:30] if c['clearances'] else ''} |"
                )
        ...
        if a["recent_activity"]:
            ...
            for c in a["recent_activity"][:10]:
                report.append(
                    f"| {c['name']} | {c['last_activity'][:10] if c['last_activity'] else ''} | {c['note_count']} |"
                )
...
def generate_zoominfo_recommendations(analyses: list) -> str:
    ...
    for a in analyses:
        if not a["gaps"]:
            report.append(
                f"- **{a['prime']}**: {a['total_contacts']} contacts, no gaps"
            )
...
def main():
    ...
    csv_files = [
        f
        for f in os.listdir(PRIME_CONTACTS_DIR)
        if f.endswith("_Contacts.csv") and f != "Master_All_Contacts.csv"
    ]
    ...
    for csv_file in sorted(csv_files):
        ...
        try:
            ...
            print(
                f"  {prime_name}: {analysis['total_contacts']} contacts, {len(analysis['gaps'])} gaps"
            )
    ...
    for a in sorted(analyses, key=lambda x: -x["total_contacts"]):
        print(
            f"{a['prime']:<25} {a['total_contacts']:>10} {a['with_programs']:>10} {a['with_clearances']:>12} {len(a['gaps']):>6}"
        )
