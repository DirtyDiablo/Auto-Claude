...
class CallNotesAnalyzer:
    ...
    def analyze_note_for_traction(self, note: str) -> dict:
        ...
        indicators = {
            'no_answer': any(x in note_lower for x in ['no answer', 'voicemail', 'vm', 'left message', 'lvm', 'did not answer', 'didn\'t answer']),
            'not_interested': any(x in note_lower for x in ['not interested', 'no interest', 'declined', 'passed', 'not looking', 'not a fit']),
            'wrong_contact': any(x in note_lower for x in ['wrong number', 'wrong person', 'no longer', 'left company', 'moved on', 'retired']),
            'no_openings': any(x in note_lower for x in ['no openings', 'no positions', 'fully staffed', 'no needs', 'not hiring', 'on hold']),
            'positive_response': any(x in note_lower for x in ['interested', 'send resume', 'send cv', 'follow up', 'scheduled', 'meeting', 'call back', 'spoke with', 'great call', 'good conversation']),
            'contract_mention': any(x in note_lower for x in ['contract', 'program', 'project', 'task order', 'idiq', 'bpa']),
            'clearance_issue': any(x in note_lower for x in ['clearance', 'poly', 'ts/sci', 'secret', 'waiting on clearance', 'clearance expired']),
            'hiring_signal': any(x in note_lower for x in ['hiring', 'looking for', 'need', 'openings', 'positions', 'ramp up', 'ramping']),
        }
    ...
    def extract_intelligence(self):
        ...
        for idx, record in enumerate(self.all_records):
            ...
            # Extract clearances
            clearance_patterns = [r'\bTS/SCI\b', r'\bTS\b', r'\bSecret\b', r'\bPoly\b', r'\bFSP\b', r'\bCI\s+Poly\b']
            ...
            if traction.get('no_answer') or traction.get('not_interested') or traction.get('no_openings'):
    ...
    def identify_gaps(self):
        ...
        program_traction = defaultdict(lambda: {'positive': 0, 'negative': 0, 'total': 0})
        ...
        contact_responses = defaultdict(lambda: {'positive': 0, 'negative': 0, 'last_contact': None})
        for contact in self.contacts_extracted:
            name = contact['name']
            if contact.get('traction', {}).get('positive_response'):
                contact_responses[name]['positive'] += 1
            elif contact.get('traction', {}).get('no_answer') or contact.get('traction', {}).get('not_interested'):
    ...
    def generate_report(self) -> dict:
        ...
        report = {
            ...
            'action_types': dict(sorted(self.action_types.items(), key=lambda x: -x[1])),
            ...
            'programs_analysis': {},
            'primes_analysis': {},
            'locations': dict(sorted(self.locations_mentioned.items(), key=lambda x: -x[1])[:30]),
            'clearances': dict(sorted(self.clearances_mentioned.items(), key=lambda x: -x[1])),
            'gap_programs': list(self.gap_programs),
            'no_traction_patterns': {},
        }

        # Program analysis
        for prog, mentions in sorted(self.programs_mentioned.items(), key=lambda x: -len(x[1])):
            report['programs_analysis'][prog] = {
                'mention_count': len(mentions),
                'sample_notes': [{'about': m['about'], 'note': m['note'][:100], 'date': m['date']} for m in mentions[:5]]
            }

        # Prime analysis
        for prime, mentions in sorted(self.primes_mentioned.items(), key=lambda x: -len(x[1])):
        ...
        no_answer_count = sum(1 for x in self.no_traction_indicators if x['indicators'].get('no_answer'))
        not_interested_count = sum(1 for x in self.no_traction_indicators if x['indicators'].get('not_interested'))
        no_openings_count = sum(1 for x in self.no_traction_indicators if x['indicators'].get('no_openings'))
        hiring_signals = sum(1 for x in self.no_traction_indicators if x['indicators'].get('hiring_signal'))
