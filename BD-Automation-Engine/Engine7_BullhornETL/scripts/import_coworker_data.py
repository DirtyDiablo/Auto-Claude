...
class BullhornXLSParser:
    ...
    def _parse_sales_activity(self, sheet):
        ...
        for row_idx in range(sheet.nrows):
            row = [
                self._cell_value(sheet, row_idx, col_idx)
            ...
            if current_section == "client_visits":
                ...
                elif "#" in row_text and any(
                    s in row_text for s in SUBMISSION_STATUSES
                ):
    ...
    def _parse_client_visit(self, row):
        ...
        for val in values:
            if val in DEPARTMENTS:
                ...
                try:
                    ...
                    logger.debug(
                        "date_parse_failed", extra={"value": val, "error": str(e)}
                    )
        ...
        name_candidates = [
            v
            for v in values
            if v not in DEPARTMENTS + CONTACT_STATUSES
    ...
    def _parse_job_or_submission(self, row):
        ...
        has_job_id = any(
            v.startswith("#") or ("#" in v and any(c.isdigit() for c in v))
            for v in values
        )
    ...
    def _parse_submission(self, row):
        ...
        for i, val in enumerate(values):
            ...
            # Client (contains "ONLY ONE" or known patterns)
                ...
                client = (
                    val.replace(" - ONLY ONE YOU ARE TO USE", "")
                    .replace("- ONLY ONE YOU ARE TO USE", "")
                    .strip()
                )
    ...
    def _parse_job_activity(self, sheet):
        ...
        for row_idx in range(sheet.nrows):
            ...
            if "No data was returned" in row_text:
                continue
            if any(
                h in row_text for h in ["Job ID", "Job Title", "Status", "Date Opened"]
            ):
                continue
...
class BDPlaybookGenerator:
    ...
    def __init__(self, parsed_data: Dict[str, Any], salesperson: str = "Colton Scurry"):
        ...
        self.playbook = {
            ...
            "summary": {},
            "contacts": {"all": [], "by_status": {}, "by_date": {}, "timeline": []},
            "clients": {"all": [], "by_activity": {}},
            "jobs": {"all": [], "by_status": {}, "by_client": {}},
            "candidates": {"all": [], "by_status": {}, "by_client": {}},
            "pipeline": {
                "submissions": [],
                "interviews": [],
                "placements": [],
        ...
        for contact in unique_contacts:
            ...
            if date_str:
                try:
                    ...
                    logger.debug(
                        "date_parse_failed", extra={"value": date_str, "error": str(e)}
                    )
    ...
    def _process_clients(self):
        ...
        by_activity = defaultdict(
            lambda: {"submissions": 0, "placements": 0, "jobs": []}
        )
        ...
        self.playbook["clients"]["by_activity"] = {
            k: dict(v) for k, v in by_activity.items()
        }
    ...
    def _process_jobs(self):
        ...
        submission_jobs = defaultdict(
            lambda: {"submissions": 0, "placements": 0, "candidates": []}
        )
        for submission in self.data.get("submissions", []):
            job_id = submission.get("job_id")
            if job_id:
        ...
        self.playbook["jobs"]["by_client"] = {
            k: dict(v) for k, v in submission_jobs.items()
        }
        print(
            f"  Processed {len(jobs)} job postings, {len(submission_jobs)} with submissions"
        )
    ...
    def _process_pipeline(self):
        ...
        print(
            f"  Pipeline: {len(self.playbook['pipeline']['placements'])} placements, "
    ...
    def _analyze_relationships(self):
        ...
        analysis["top_clients"] = [
            {
                "name": c[0],
                "submissions": c[1].get("submissions", 0),
        ...
        sorted_jobs = sorted(
            job_activity.items(), key=lambda x: x[1].get("submissions", 0), reverse=True
        )
        analysis["hot_jobs"] = [
            {
                "job_id": j[0],
                "title": j[1].get("title", ""),
                "client": j[1].get("client", ""),
                "submissions": j[1].get("submissions", 0),
            }
    ...
    def _generate_recommendations(self):
        ...
        new_leads = [
            c for c in contacts if c.get("status") in ["New Lead", "New Contact"]
        ]
        if new_leads:
            recommendations.append(
                {
    ...
    def _build_summary(self):
        ...
        self.playbook["summary"] = {
            ...
            "contact_statuses": {
                k: len(v) for k, v in self.playbook["contacts"]["by_status"].items()
            },
...
def generate_markdown_report(playbook: Dict) -> str:
    ...
    for client in playbook["analysis"].get("top_clients", [])[:15]:
        lines.append(
            f"| {client['name']} | {client['submissions']} | {client['placements']} |"
        )
    ...
    for job in playbook["analysis"].get("hot_jobs", [])[:15]:
        lines.append(
            f"| {job['job_id']} | {job.get('title', 'N/A')[:30]} | {job.get('client', 'N/A')[:20]} | {job['submissions']} |"
        )

    # Pipeline Section
    lines.extend(
        ["", "---", "", "## Active Pipeline", "", "### Interviews in Progress", ""]
    )
    ...
    lines.extend(["", "### Recent Placements", ""])
    ...
    # Contacts Section
    lines.extend(["", "---", "", "## Contact Directory", "", "### By Status", ""])
    ...
    # Recruiter Partners
    lines.extend(["---", "", "## Recruiter Partners", ""])
