...
def generate_instructions_md(playbook: Dict, output_dir: Path):
    ...
    for client in playbook['analysis'].get('top_clients', [])[:5]:
        lines.append(f"- **{client['name']}** ({client['submissions']} submissions, {client['placements']} placements)")
...
def generate_master_csv(playbook: Dict, parsed: Dict, output_dir: Path):
    ...
    if rows:
        fieldnames = ['Record_Type', 'Date', 'Client', 'Job_ID', 'Job_Title', 'Candidate',
                      'Status', 'Recruiter', 'Salesperson', 'Department', 'Contact_Name', 'Notes']
...
def generate_programs_csv(playbook: Dict, parsed: Dict, output_dir: Path):
    ...
    for job in parsed.get('jobs', []):
        ...
        if job_id:
            ...
            job_stats[job_id]['title'] = job.get('title', '') or job_stats[job_id]['title']
            job_stats[job_id]['client'] = job.get('client', '') or job_stats[job_id]['client']
            job_stats[job_id]['status'] = job.get('status', '') or job_stats[job_id]['status']
            job_stats[job_id]['type'] = job.get('type', '') or job_stats[job_id]['type']
            job_stats[job_id]['location'] = job.get('location', '') or job_stats[job_id]['location']
    ...
    for sub in parsed.get('submissions', []):
        ...
        if job_id:
            job_stats[job_id]['job_id'] = job_id
            job_stats[job_id]['title'] = sub.get('title', '') or job_stats[job_id]['title']
            job_stats[job_id]['client'] = sub.get('client', '') or job_stats[job_id]['client']
    ...
    if rows:
        fieldnames = ['Job_ID', 'Title', 'Client', 'Status', 'Type', 'Location',
                      'Date_Opened', 'Total_Submissions', 'Interviews', 'Placements', 'Top_Candidates']
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(rows)
...
def generate_contacts_csv(playbook: Dict, parsed: Dict, output_dir: Path):
    ...
    if rows:
        fieldnames = ['Name', 'First_Name', 'Last_Name', 'Status', 'Type', 'Department', 'Date_Added', 'Salesperson']
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(rows)
...
def generate_org_chart_csv(playbook: Dict, parsed: Dict, output_dir: Path):
    ...
    if rows:
        fieldnames = ['Client', 'Contact_Name', 'Role', 'Job_ID', 'Job_Title', 'Status', 'Associated_Recruiter']
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(rows)
...
def generate_program_md_files(playbook: Dict, parsed: Dict, job_stats: Dict, output_dir: Path):
    """Generate individual MD files for each major program."""
    programs_dir = output_dir / "programs"
    programs_dir.mkdir(exist_ok=True)

    # Get top programs by activity
    sorted_jobs = sorted(job_stats.items(), key=lambda x: x[1]['submissions'], reverse=True)
    ...
    for job_id, stats in sorted_jobs[:30]:  # Top 30 programs
        ...
        job_submissions = [s for s in parsed.get('submissions', []) if s.get('job_id') == job_id]
        ...
        if 'Placed' in by_status:
            ...
            for sub in by_status['Placed']:
                lines.append(f"- **{sub.get('candidate', 'Unknown')}** - Placed {sub.get('date', '')}")
            lines.append("")
        ...
        if interview_keys:
            ...
            for key in interview_keys:
                for sub in by_status[key]:
                    lines.append(f"- **{sub.get('candidate', 'Unknown')}** - {key} ({sub.get('date', '')})")
            lines.append("")
        ...
        if 'Client Submission' in by_status:
            ...
            for sub in by_status['Client Submission'][:20]:
                lines.append(f"- {sub.get('candidate', 'Unknown')} - Submitted {sub.get('date', '')}")
            if len(by_status['Client Submission']) > 20:
                lines.append(f"- ... and {len(by_status['Client Submission']) - 20} more")
            lines.append("")

        # Recruiters involved
        recruiters = set(s.get('recruiter', '') for s in job_submissions if s.get('recruiter'))
        if recruiters:
            lines.append("---")
            lines.append("")
    ...
    print(f"  Created: programs/ folder ({len(list(programs_dir.glob('*.md')))} program files)")
