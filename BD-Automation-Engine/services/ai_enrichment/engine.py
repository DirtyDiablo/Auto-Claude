...
class EnrichmentEngine:
    ...
    def enrich_job(self, job: Dict) -> Dict:
        ...
        title = NotionClient.extract_title(
            props.get("Job Title", props.get("Name", props.get("title", {})))
        )
        company = NotionClient.extract_rich_text(props.get("Company", {}))
        location = NotionClient.extract_rich_text(props.get("Location", {}))
        description = NotionClient.extract_rich_text(
            props.get("Job Description", props.get("Description", {}))
        )
        clearance = NotionClient.extract_select(
            props.get("Clearance Level", props.get("Clearance", {}))
        )

        # 1. Map location to programs
        mapped_programs = self._map_location_to_programs(location)
        if mapped_programs:
            enriched["Mapped Programs"] = NotionClient.build_rich_text(
                ", ".join(mapped_programs)
            )
        ...
        if self.client and description:
            ai_analysis = self._ai_analyze_job(
                title, company, location, description, clearance
            )
            if ai_analysis:
                if ai_analysis.get("key_skills"):
                    enriched["Key Skills"] = NotionClient.build_multi_select(
                        ai_analysis["key_skills"][:10]
                    )
                if ai_analysis.get("program_alignment"):
                    enriched["Program Alignment"] = NotionClient.build_rich_text(
                        ai_analysis["program_alignment"]
                    )
                if ai_analysis.get("win_themes"):
                    enriched["Win Themes"] = NotionClient.build_rich_text(
                        ai_analysis["win_themes"]
                    )
                if ai_analysis.get("confidence_score"):
                    enriched["AI Confidence Score"] = NotionClient.build_number(
    ...
    def enrich_contact(self, contact: Dict) -> Dict:
        ...
        # Extract existing values
        name = NotionClient.extract_title(
            props.get("Name", props.get("Contact Name", {}))
        )
        title = NotionClient.extract_rich_text(
            props.get("Title", props.get("Job Title", {}))
        )
        company = NotionClient.extract_rich_text(
            props.get("Company", props.get("Organization", {}))
        )
            ...
            if ai_enrichment:
                if ai_enrichment.get("outreach_approach"):
                    ...
                    if isinstance(approach, list):
                        approach = "\n".join(approach)
                    enriched["Outreach Approach"] = NotionClient.build_rich_text(
                        str(approach)
                    )
                if ai_enrichment.get("talking_points"):
                    ...
                    enriched["Talking Points"] = NotionClient.build_rich_text(
                        str(points)
                    )
    ...
    def enrich_opportunity(self, opportunity: Dict) -> Dict:
        ...
        # Extract existing values
        title = NotionClient.extract_title(
            props.get("Opportunity Name", props.get("Name", {}))
        )
        agency = NotionClient.extract_select(props.get("Agency", {}))
        value = NotionClient.extract_number(
            props.get("Estimated Value", props.get("Contract Value", {}))
        )
        ...
        if value and win_prob:
            ...
            enriched["Revenue Potential"] = NotionClient.build_number(
                round(revenue_potential, 2)
            )
        ...
        if self.client and description:
            ...
            if ai_analysis:
                if ai_analysis.get("competitive_landscape"):
                    enriched["Competitive Landscape"] = NotionClient.build_rich_text(
                        ai_analysis["competitive_landscape"]
                    )
                if ai_analysis.get("win_strategy"):
                    enriched["Win Strategy"] = NotionClient.build_rich_text(
                        ai_analysis["win_strategy"]
                    )
...
Format as clean Markdown."""
    ...
    def _calculate_bd_score(
        self, location: str, clearance: Optional[str], description: str, company: str
    ) -> int:
        """
    ...
    def _calculate_win_probability(
        self, agency: Optional[str], value: Optional[float], description: str
    ) -> int:
...
Only return valid JSON, no other text."""
    ...
    def _ai_analyze_opportunity(
        self, title: str, agency: Optional[str], description: str
    ) -> Optional[Dict]:
...
Only return valid JSON, no other text."""
    ...
    def _generate_basic_playbook(
        self, title: str, company: str, location: str, contacts: List[Dict]
    ) -> str:
