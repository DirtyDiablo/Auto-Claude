#!/usr/bin/env python3
"""
DCGS Contacts Retry - Retry enrichment for contacts missing Tier/Contact Value
Targets the ~120 records that failed during initial enrichment
"""
...
def query_unenriched_contacts(cursor=None):
    """Query contacts that don't have Tier set"""
    ...
    # Filter for records where Tier is empty
    body = {
        "page_size": 50,
        "filter": {"property": "Tier", "select": {"is_empty": True}},
    }
    if cursor:
...
def run_dcgs_retry():
    """Run enrichment retry for failed records"""
    print("DCGS Contacts Retry Enrichment", flush=True)
    print("=" * 60, flush=True)
    print("Finding contacts without Tier set...", flush=True)
    print("", flush=True)
    ...
    while True:
        ...
        # Query unenriched contacts
        result = query_unenriched_contacts(cursor)
        ...
        if not pages:
            print("  No more unenriched contacts found", flush=True)
            break

        # Process this batch
        batch_success = 0
        batch_failed = 0
        print(f"  Found {len(pages)} unenriched contacts, updating...", flush=True)
