...
class ReferenceDataLoader:
    ...
    def load_naics(self):
        ...
        try:
            ...
            if (
                "principalnaicscode" in df.columns
                and "principalnaicscode_label" in df.columns
            ):
                self.naics_lookup = dict(
                    zip(
    ...
    def load_psc(self):
        ...
        psc_path = os.path.join(
            self.lookup_dir, "Lookup-Tables", "productorservice", "PSCAtransition.csv"
        )
        ...
        try:
            ...
            if (
                "ProductOrServiceCode" in df.columns
                and "ProductOrServiceCodeDescription" in df.columns
            ):
                self.psc_lookup = dict(
                    zip(
                ...
                cols = df.columns.tolist()
                self.psc_lookup = dict(
                    zip(df[cols[0]].astype(str).str.strip(), df[cols[1]])
                )
...
class EnhancedFederalProgramEnricherV3:
    ...
    def __init__(
        self, input_csv: str, contracts_csv: str, output_csv: str, sam_api_key: str
    ):
    ...
    def load_and_merge_contracts(self) -> pd.DataFrame:
        ...
        print(f"\nMerge results:")
        print(
            f"  {contracts_found}/{len(merged)} programs now have contract numbers ({contracts_found / len(merged) * 100:.1f}%)"
        )
    ...
    def enrich_basic(self, program: Dict) -> Dict:
        ...
        enriched["Functional Areas"] = (
            "; ".join(found_functional) if found_functional else ""
        )
    ...
    def query_fpds_for_contract(self, piid: str) -> Dict:
        ...
        try:
            ...
            params = {"FEEDNAME": "PUBLIC", "q": piid, "start": "1"}
    ...
    def enrich_with_fpds(self, program: Dict) -> Dict:
        ...
        if fpds_data:
            ...
            enriched["Contract Effective Date (FPDS)"] = fpds_data.get(
                "effectiveDate", ""
            )
            enriched["Current Completion Date (FPDS)"] = fpds_data.get(
                "currentCompletionDate", ""
            )
            enriched["Ultimate Completion Date (FPDS)"] = fpds_data.get(
                "ultimateCompletionDate", ""
            )
            enriched["Base Contract Value (FPDS)"] = fpds_data.get(
                "baseAndExercisedOptionsValue", ""
            )
            enriched["Base + Options Value (FPDS)"] = fpds_data.get(
                "baseAndAllOptionsValue", ""
            )

            city = fpds_data.get("placeOfPerformanceCity", "")
            state = fpds_data.get("placeOfPerformanceStateCode", "")
            enriched["Performance Location (FPDS)"] = (
    ...
    def query_calc_api(self, labor_category: str) -> Optional[Dict]:
        ...
        try:
            ...
            params = {"q": labor_category, "limit": 10}
            ...
            self.stats["errors"].append(
                f"CALC API error for {labor_category}: {str(e)}"
            )
            return None
    ...
    def run_enrichment(self, process_count: int = None):
        ...
        print(
            f"  Contract Numbers Merged: {self.stats['contracts_merged']} ({self.stats['contracts_merged'] / self.stats['total_processed'] * 100:.1f}%)"
        )
        print(
            f"  FPDS Records: {self.stats['fpds_enriched']} ({self.stats['fpds_enriched'] / self.stats['total_processed'] * 100:.1f}%)"
        )
        print(
            f"  NAICS Descriptions: {self.stats['naics_enriched']} ({self.stats['naics_enriched'] / self.stats['total_processed'] * 100:.1f}%)"
        )
        print(
            f"  PSC Descriptions: {self.stats['psc_enriched']} ({self.stats['psc_enriched'] / self.stats['total_processed'] * 100:.1f}%)"
        )
        print(
            f"  CALC Labor Rates: {self.stats['calc_enriched']} ({self.stats['calc_enriched'] / self.stats['total_processed'] * 100:.1f}%)"
        )
        ...
        if self.stats["errors"]:
            error_log = os.path.join(
                os.path.dirname(self.output_csv), "enrichment-errors-v3.log"
            )
...
def main():
    ...
    enricher = EnhancedFederalProgramEnricherV3(
        input_csv, contracts_csv, output_csv, SAM_API_KEY
    )
