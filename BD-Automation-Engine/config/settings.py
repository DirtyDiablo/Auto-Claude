...
class Settings(BaseSettings):
    ...
    notion_api_key: str = Field(
        default="",
        description="Alias for notion_token"
    )
    ...
    notion_db_gdit_jobs: str = Field(
        default="2563119e7914442cbe0fb86904a957a1",
        description="GDIT Jobs database ID"
    )
    ...
    hub_api_key: str = Field(
        default="",
        description="Hub API authentication key"
    )
    ...
    n8n_webhook_url: str = Field(
        default="",
        description="n8n webhook URL for job intake"
    )
    ...
    proxycurl_api_key: str = Field(
        default="",
        description="ProxyCurl API key for LinkedIn enrichment"
    )
    reacher_api_key: str = Field(
        default="",
        description="Reacher API key for email verification"
    )
    ...
    medium_confidence_threshold: float = Field(
        default=0.50,
        description="Medium confidence match threshold"
    )
    bd_tier_hot_min: int = Field(
        default=80,
        description="Minimum score for Hot BD tier"
    )
    bd_tier_warm_min: int = Field(
        default=50,
        description="Minimum score for Warm BD tier"
    )
    ...
    log_file: str = Field(
        default="outputs/Logs/bd_automation.log",
