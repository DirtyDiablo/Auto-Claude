...
class HiringForecaster:
    ...
    async def forecast_program_hiring(
        ...
        forecast_points = self._project_forward(
            weekly_counts, horizon_days, trend, strength
        )
        forecasted_rate = (
            forecast_points[-1].predicted if forecast_points else current_rate
        )
    ...
    async def detect_ramp_signals(
        ...
        for program, items in by_program.items():
            ...
            baseline = (
                sum(weekly[:-4]) / max(len(weekly) - 4, 1)
                if len(weekly) > 4
                else recent_avg
            )
            ...
            if trend == "increasing" and strength > 0.3 and recent_avg > baseline * 1.5:
                ...
                if new_roles:
                    evidence.append(
                        f"New role types appearing: {', '.join(list(new_roles)[:3])}"
                    )
            ...
            # Wind-down: declining postings
                ...
                trend == "decreasing" and strength > 0.3 and recent_avg < baseline * 0.5
            ):
                evidence = [
    ...
    def _get_program_data(self, program: str) -> List[dict]:
        return [
            ...
            if program.lower() in str(d.get("program", "")).lower()
        ]
    ...
    def _get_location_data(self, location: str) -> List[dict]:
        return [
            d
            for d in self._historical
            if location.lower() in str(d.get("location", "")).lower()
        ]
    ...
    def _get_role_data(self, role: str) -> List[dict]:
        return [
            d
            for d in self._historical
            if role.lower() in str(d.get("role", d.get("title", ""))).lower()
        ]
    ...
    def _detect_trend(self, weekly: List[float]) -> Tuple[str, float]:
        ...
        if not NP_AVAILABLE:
            ...
            second_half = sum(weekly[len(weekly) // 2 :]) / max(
                len(weekly) - len(weekly) // 2, 1
            )
    ...
    def _detect_seasonal_pattern(self, weekly: List[float]) -> str:
        ...
        if (
            max_q == 3
            and quarters[2] > sum([quarters[0], quarters[1], quarters[3]]) / 3 * 1.3
        ):
            return "Q3_peak"
        return "steady"

    def _project_forward(
        self, weekly: List[float], horizon_days: int, trend: str, strength: float
    ) -> List[ForecastPoint]:
    ...
    def _empty_forecast(
        self, entity: str, entity_type: str, horizon_days: int
    ) -> HiringForecast:
