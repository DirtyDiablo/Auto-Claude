...
try:
    ...
    for col in collections:
        name = col["name"]
        info_response = urllib.request.urlopen(
            f"http://localhost:6333/collections/{name}", timeout=5
        )
...
print(
    f"  {'Collection':<15} {'Source':>12} {'Indexed':>12} {'Remaining':>12} {'Progress':>10}"
)
print(f"  {'-' * 65}")
...
for collection in ["contacts", "activities", "jobs", "documents", "programs"]:
    ...
    if collection == "documents":
        ...
        print(
            f"  {collection:<15} {'(files)':>12} {indexed:>12,} {'-':>12} {'[DONE]':>10}"
        )
    elif collection == "programs":
        # Programs from multiple sources
        print(
            f"  {collection:<15} {'(multi)':>12} {indexed:>12,} {'-':>12} {'[DONE]':>10}"
        )
    elif source > 0:
        ...
        if pct >= 95:
            ...
            status = f"[{pct:.0f}%]"
        print(
            f"  {collection:<15} {source:>12,} {indexed:>12,} {remaining:>12,} {status:>10}"
        )
    else:
...
if contacts_remaining > 0:
    print(
        f"  1. [NEEDED] Index {contacts_remaining:,} more contacts from candidates table"
    )
else:
