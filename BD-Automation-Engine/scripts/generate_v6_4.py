#!/usr/bin/env python3
"""Generate PTS Data Architecture Explorer V6.4 — search results list, nav history, group labels, hover tooltips."""
import json
import os
...
function DetailPanel({ data, onClose, onNavigateToEntity, navHistory, onBack }) {
  ...
  // Auto-scroll to top on entity change
...
function App() {
  ...
  // Navigate to entity (from detail panel or search)
  const navigateToEntity = useCallback(entityId => {
    ...
    if (node) {
      ...
      setFocusId(entityId);
      setNavHistory(prev => {
        // If clicking back in crumbs, truncate forward history
  ...
  // Handle initial node selection (from graph click)
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.key === 'Escape') { setSel(null); setFocusId(null); setSq(''); setNavHistory([]); setHoverData(null); }
  ...
  return html`<div style=${{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
    <div className="top-bar">
      ...
      <span className="top-bar-ver">V6.4</span>
      <div style=${{ width: 1, height: 20, background: 'var(--border-0)' }}></div>
      <div style=${{ display: 'flex', gap: 0 }}>${tabs.map(t => html`<button key=${t.id} className=${'tab-btn' + (tab === t.id ? ' active' : '')} onClick=${() => { setTab(t.id); setSel(null); setFocusId(null); setNavHistory([]); }}>${t.label}</button>`)}</div>
    ...
    <div style=${{ display: 'flex', flex: 1, overflow: 'hidden' }}>
      ${tab === 'graph' && html`<${CategorySidebar} activeCategories=${ac} onToggle=${toggleCat} searchQuery=${sq} onSearchChange=${setSq} useGrouped=${grouped} onToggleGrouped=${() => setGrouped(g => !g)} searchMatches=${searchMatches} onSearchResultClick=${handleSearchResultClick} />`}
    ...
    ${tab === 'graph' && html`<div className="kb-hint"><span><span className="kb-key">Esc</span> Clear</span><span><span className="kb-key">/</span> Search</span><span><span className="kb-key">Click</span> Focus</span>${navHistory.length > 1 && html`<span><span className="kb-key">${'\u232B'}</span> Back</span>`}<span><span className="kb-key">Scroll</span> Zoom</span></div>`}
  </div>`;
}
...
<title>PTS Data Architecture Explorer V6.4</title>
...
outpath = os.path.join(BASE, 'PTS_DATA_ARCHITECTURE_EXPLORER_V6.4.html')
with open(outpath, 'w', encoding='utf-8') as f:
    f.write(output)

print(f"V6.4 written: {len(output):,} chars ({len(output)//1024}KB)")
