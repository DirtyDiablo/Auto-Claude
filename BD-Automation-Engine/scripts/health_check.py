...
def format_report(qdrant, api, neo4j, vite, mem0, git):
    ...
    if qdrant["status"] == "✅":
        q_detail = ", ".join(
            f"{k}:{v:,}"
            for k, v in sorted(qdrant["detail"].items(), key=lambda x: -x[1])
        )
    ...
    api_summary = f"Hub API {api['status']}" + (
        f" healthy" if api["status"] == "✅" else f" {api.get('error', '')}"
    )
    neo4j_summary = f"Neo4j {neo4j['status']}" + (
        f" {neo4j.get('nodes', 0)} nodes, {neo4j.get('relationships', 0)} rels"
        if neo4j["status"] == "✅"
        else f" {neo4j.get('error', '')}"
    )
    vite_summary = f"Vite {vite['status']}" + (
        f" on :5173" if vite["status"] == "✅" else f" {vite.get('note', '')}"
    )
