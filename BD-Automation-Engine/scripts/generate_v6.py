#!/usr/bin/env python3
"""Generate PTS Data Architecture Explorer V6 HTML from V5 data."""
import json
import os

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

with open(os.path.join(BASE, 'outputs', 'v6_arch_data.json'), 'r') as f:
    v6 = json.load(f)
...
.react-flow__edge-text{fill:var(--text-3)!important;font-size:8px!important;font-family:'JetBrains Mono',monospace!important}
.react-flow__edge-textbg{fill:var(--bg-0)!important}
.entity-node{border-radius:4px;overflow:hidden;font-size:11px;cursor:pointer;transition:all .2s;border:1px solid transparent;min-width:185px;background:var(--bg-2)}
...
.entity-header{padding:6px 10px;display:flex;justify-content:space-between;align-items:center}
.entity-header .name{color:#fff;font-weight:600;font-size:11px;letter-spacing:.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:130px}
.record-badge{background:rgba(0,0,0,.35);color:rgba(255,255,255,.75);padding:1px 6px;border-radius:3px;font-size:9px;font-family:'JetBrains Mono',monospace;font-weight:500}
.entity-body{padding:4px 10px 5px;display:flex;align-items:center;gap:6px;background:rgba(0,0,0,.12)}
...
.flow-node{border-radius:4px;overflow:hidden;font-size:11px;border:1px solid rgba(255,255,255,.06)}
...
.integ-node{padding:12px 16px;text-align:center;font-size:11px;border:1px solid rgba(255,255,255,.06);border-radius:4px}
...
.dash-card{background:var(--bg-1);border:1px solid var(--border-0);border-radius:6px;padding:20px;transition:border-color .2s}
...
// === ELK Layout ===
async function layoutNodes(nodes, edges, dir = 'DOWN') {
  const graph = {
    id: 'root',
    layoutOptions: {
...
// === Entity Graph View ===
function EntityGraphView({ onSelectNode, activeCategories, searchQuery }) {
  ...
  useEffect(() => {
    ...
    const se = filtered.edges.map(e => ({
      ...e, type: 'smoothstep',
      style: { stroke: 'rgba(148,163,184,0.18)', strokeWidth: 1.5 },
      markerEnd: { type: MarkerType.ArrowClosed, color: 'rgba(148,163,184,0.25)', width: 12, height: 12 },
      label: e.label || (e.data && e.data.label) || '',
      labelStyle: { fill: '#475569', fontSize: 8, fontFamily: 'JetBrains Mono' },
      labelBgStyle: { fill: '#080c16', fillOpacity: 0.9 },
      labelBgPadding: [4, 2],
    }));
    layoutNodes(filtered.nodes, se).then(ln => {
      setNodes(ln);
      setEdges(se);
      setLoading(false);
  ...
  return html`
    <${ReactFlow} nodes=${nodes} edges=${edges} onNodesChange=${onNodesChange} onEdgesChange=${onEdgesChange}
      onNodeClick=${onNodeClick} nodeTypes=${nodeTypes} fitView fitViewOptions=${{ padding: 0.1 }}
      minZoom=${0.08} maxZoom=${2.5} proOptions=${{ hideAttribution: true }}>
...
function DataFlowView() {
  ...
  useEffect(() => {
    ...
    layoutNodes(fn, fe, 'RIGHT').then(ln => {
      setNodes(ln); setEdges(fe); setLoading(false);
  ...
  return html`<div className="dash-grid">
    <div className="dash-card"><h3>System Overview</h3>
      <div style=${{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <div><div className="stat-big">${d.multiProjectEntities}</div><div className="stat-label">Multi-Project</div></div>
      </div>
    </div>
...
// === Category Sidebar ===
function CategorySidebar({ activeCategories, onToggle, searchQuery, onSearchChange }) {
  ...
  return html`<div className="cat-sidebar">
    <div className="cat-sidebar-hdr">
      ...
      <div style=${{ display: 'flex', gap: '6px', marginTop: '8px' }}><button className="toolbar-btn" onClick=${() => onToggle('__ALL__')}>${allOn ? 'Hide All' : 'Show All'}</button></div>
    </div>
...
function App() {
  ...
  return html`<div style=${{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
    <div className="top-bar">
      ...
      <span className="top-bar-ver">V6</span>
    ...
    <div style=${{ display: 'flex', flex: 1, overflow: 'hidden' }}>
      ...
      <div style=${{ flex: 1, position: 'relative' }}>
        ${tab === 'graph' && html`<${ReactFlowProvider}><${EntityGraphView} onSelectNode=${setSel} activeCategories=${ac} searchQuery=${sq} /></${ReactFlowProvider}>`}
...
<title>PTS Data Architecture Explorer V6</title>
...
outpath = os.path.join(BASE, 'PTS_DATA_ARCHITECTURE_EXPLORER_V6.html')
with open(outpath, 'w', encoding='utf-8') as f:
    f.write(output)

print(f"V6 written: {len(output):,} chars ({len(output)//1024}KB)")
