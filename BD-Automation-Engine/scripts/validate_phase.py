...
def validate_phase_1():
    ...
    # Test vector search
    code, data = http_post(
        "http://localhost:8100/search", {"query": "DCGS", "limit": 3}
    )
    results.append(
        check(
            "POST /search working",
            code == 200,
            f"returned {len(data.get('results', [])) if isinstance(data, dict) else '?'} results",
        )
    )

    # Test smart search
    code, data = http_post(
        "http://localhost:8100/ask/smart", {"query": "What programs does GDIT run?"}
    )
    ...
    results.append(
        check(
            "Vite proxy /api/* → :8100",
            code == 200,
            "proxy working" if code == 200 else "check vite.config.ts",
        )
    )
    ...
    results.append(
        check("API client file exists", found_api, "src/lib/api.ts or equivalent")
    )
...
def validate_phase_2():
    ...
    code, data = http_post(
        ...
        {"query": "program manager", "collection_name": "contacts", "limit": 5},
    )
    ...
    code, data = http_post(
        "http://localhost:8100/search",
        {"query": "DCGS", "collection_name": "programs", "limit": 5},
    )
    has_results = isinstance(data, dict) and len(data.get("results", [])) > 0
    results.append(
        check(
...
def validate_phase_3():
    ...
    results.append(
        check(
            "Outreach API (:8300) responding",
            code == 200,
            "running"
            if code == 200
            else "start with: uvicorn src.outreach.api:app --port 8300",
        )
    )
    ...
    if code == 200 and isinstance(data, list):
        results.append(
            check("Agent task history accessible", True, f"{len(data)} tasks")
        )
    else:
    ...
    code, data = http_post(
        "http://localhost:8100/search",
        {"query": "network engineer TS/SCI", "collection_name": "jobs", "limit": 5},
    )
    has_jobs = isinstance(data, dict) and len(data.get("results", [])) > 0
    results.append(
        check(
            "Jobs collection searchable",
            has_jobs,
...
def main():
    ...
    if passed == total:
        print(
            f"{Colors.GREEN}✅ Phase {phase} VALIDATED — ready for Phase {phase + 1}{Colors.RESET}"
        )
    elif passed > total * 0.7:
        print(
            f"{Colors.YELLOW}⚠️ Phase {phase} MOSTLY COMPLETE — review failures above{Colors.RESET}"
        )
    else:
        print(
            f"{Colors.RED}❌ Phase {phase} INCOMPLETE — fix failures before proceeding{Colors.RESET}"
        )

    print(f"\n{Colors.BOLD}COPY-PASTE FOR ORCHESTRATOR:{Colors.RESET}")
    print(
        f"Phase {phase} validation: {passed}/{total} passed. "
        + (
            "Ready for next phase."
            if passed == total
            else f"Failing: {', '.join(r for r, ok in zip([str(i) for i in range(len(results))], results) if not ok)}"
        )
    )
