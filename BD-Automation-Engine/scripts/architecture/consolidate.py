...
def main():
    ...
    parser.add_argument(
        ...
        help='Only generate V4 HTML visualization'
    )
    parser.add_argument(
        '--discover-rels', action='store_true',
    ...
    sources = []
    for label, path in [('BD-Engine', BD_PATH), ('Data-Scraper', DS_PATH), ('N8N-Builder', N8N_PATH)]:
        if not path.exists():
    ...
    if args.discover_rels:
        auto_count = sum(1 for r in relationships
                         if 'AUTO_DISCOVERED' in r.get('contributed_by', []))
    ...
    if infrastructure.get('qdrant_collections'):
        infra_counts.append(f"Qdrant: {len(infrastructure['qdrant_collections'])} collections")
    if infrastructure.get('sqlite_databases'):
        infra_counts.append(f"SQLite: {len(infrastructure['sqlite_databases'])} databases")
    ...
    # Category breakdown
    from .name_map import CATEGORY_DEFS, assign_category
    print("\n  Category breakdown:")
