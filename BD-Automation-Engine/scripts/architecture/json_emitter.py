        ...
        "properties": [
            {
                ...
                **(
                    {
                        k: p[k]
                        for k in ("fk_target", "enum_values", "vector_dim")
                        if k in p
                    }
                ),
            }
    ...
    stats.append(
        {
            ...
            "properties": sum(
                len(e.get("properties", [])) for e in V2_ONLY_ENTITIES.values()
            ),
...
def generate_unified_json(
    ...
    return {
        ...
        "entities": [
            _build_entity_output(name, ent) for name, ent in sorted(entities.items())
        ],
        "relationships": [
            {
                "from": r["from"],
