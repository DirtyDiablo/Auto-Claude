...
def build_architecture() -> dict:
    ...
    arch["scan_stats"] = {
        ...
        "neo4j_node_types": len(
            arch["databases"]["neo4j_graph"]["node_types"]
        ),
        "neo4j_relationship_types": len(
            arch["databases"]["neo4j_graph"]["relationship_types"]
        ),
        "notion_databases_documented": len(
            arch["databases"]["notion_databases"]
        ),
        ...
        "streaming_schemas_documented": len(
            arch["databases"]["streaming_schemas"]
        ),
        "src_api_routers": 32,
...
def _entity_contact():
    return {
        ...
        "storage": ["qdrant:contacts", "sqlite:bullhorn_master.db/candidates", "neo4j:Person"],
        "properties": [
            {"name": "id", "type": "string", "source": "bullhorn", "note": "Bullhorn candidate ID"},
            {"name": "first_name", "type": "string", "source": "bullhorn"},
            {"name": "last_name", "type": "string", "source": "bullhorn"},
            {"name": "full_name", "type": "string", "source": "derived", "note": "COALESCE(full_name, first+last)"},
            {"name": "name", "type": "string", "source": "qdrant", "note": "Display name in vector store"},
            {"name": "title", "type": "string", "source": "bullhorn", "note": "Job/position title"},
            {"name": "company", "type": "string", "source": "bullhorn", "note": "COALESCE(company_name, current_employer)"},
            {"name": "program", "type": "string", "source": "engine3", "note": "Matched federal program"},
            {"name": "email", "type": "string", "source": "bullhorn"},
            {"name": "phone", "type": "string", "source": "bullhorn", "note": "COALESCE(phone, mobile)"},
            ...
            {"name": "tier", "type": "string", "source": "engine3", "example": "Tier 1", "note": "Computed by classify_tier()"},
            {"name": "hierarchy_tier", "type": "string", "source": "engine3", "values": ["Tier 1 - Executive", "Tier 2 - Director", "Tier 3 - Program Leadership", "Tier 4 - Management", "Tier 5 - Senior IC", "Tier 6 - Individual Contributor"]},
            {"name": "bd_priority", "type": "string", "source": "engine5", "values": ["Critical", "High", "Medium", "Standard"]},
            {"name": "clearance_level", "type": "string", "source": "bullhorn", "values": ["TS/SCI w/ Full Scope Poly", "TS/SCI w/ CI Poly", "TS/SCI w/ Poly", "TS/SCI", "Top Secret", "Secret", "Public Trust", "None"]},
            {"name": "location_hub", "type": "string", "source": "engine3", "values": ["Hampton Roads", "San Diego Metro", "DC Metro", "Dayton/Wright-Patt", "OCONUS", "Other CONUS"]},
            {"name": "outreach_sequence", "type": "string", "source": "engine3", "note": "A-D outreach tier"},
            ...
            {"name": "notes", "type": "text", "source": "bullhorn", "note": "custom_text1"},
            {"name": "source", "type": "string", "source": "bullhorn", "note": "source_file"},
            {"name": "source_db", "type": "string", "source": "etl", "example": "bullhorn_master"},
            ...
            {"name": "_embedding_model", "type": "string", "source": "qdrant_meta", "example": "text-embedding-3-small"},
        ],
        "aliases": [
            {"canonical": "company", "alias": "current_employer", "source": "Bullhorn CSV"},
            ...
            {"canonical": "linkedin_url", "alias": "linkedin", "source": "Qdrant payload"},
            {"canonical": "id", "alias": "bullhorn_candidate_id", "source": "Bullhorn DB"},
            {"canonical": "id", "alias": "contact_id", "source": "API response"},
            {"canonical": "tier", "alias": "hierarchy_tier", "source": "Engine3 output"},
...
def _entity_program():
    return {
        ...
        "sources": ["Bullhorn CRM", "Federal Programs CSV", "Engine2_ProgramMapping", "Qdrant"],
        "storage": ["qdrant:programs", "sqlite:bullhorn_master.db/programs", "neo4j:Program", "csv:Federal_Programs_MASTER_V4.csv"],
        "properties": [
            ...
            {"name": "agency", "type": "string", "source": "multiple", "note": "Agency Owner"},
            ...
            {"name": "key_locations", "type": "list[str]", "source": "csv", "note": "Semicolon-separated"},
            ...
            {"name": "keywords", "type": "list[str]", "source": "csv", "note": "Keywords/Signals"},
            ...
            {"name": "bd_priority", "type": "string", "source": "engine5", "values": ["Critical", "High", "Medium", "Low"]},
            {"name": "hiring_velocity", "type": "string", "source": "neo4j", "values": ["High", "Medium", "Low", "None"]},
        ...
        "aliases": [
            ...
            {"canonical": "prime_contractor", "alias": "Prime Contractor", "source": "Notion"},
            {"canonical": "clearance_req", "alias": "Clearance Requirements", "source": "CSV"},
...
def _entity_job():
    return {
        ...
        "sources": ["Apify Scraper", "Bullhorn CRM", "Engine2_ProgramMapping", "Engine5_Scoring"],
        ...
        "properties": [
            ...
            {"name": "match_confidence", "type": "float", "source": "engine2", "note": "0.0-1.0"},
            {"name": "match_type", "type": "string", "source": "engine2", "values": ["direct", "fuzzy", "inferred"]},
            {"name": "bd_priority_score", "type": "integer", "source": "engine5", "note": "0-100"},
            {"name": "priority_tier", "type": "string", "source": "engine5", "values": ["Hot", "Warm", "Cold"]},
            {"name": "match_signals", "type": "list[str]", "source": "engine2"},
            {"name": "pipeline_stage", "type": "string", "source": "api", "values": ["scraped", "mapped", "contacts_found", "outreach_active", "meeting_set", "req_obtained"]},
            {"name": "description", "type": "text", "source": "multiple"},
            {"name": "skills", "type": "text", "source": "bullhorn"},
            {"name": "employment_type", "type": "string", "source": "bullhorn", "values": ["Full-Time", "Contract", "Contract-to-Hire", "Part-Time"]},
            {"name": "status", "type": "string", "source": "bullhorn"},
        ...
        "aliases": [
            {"canonical": "company", "alias": "client_corporation", "source": "Bullhorn DB"},
            {"canonical": "clearance", "alias": "clearance_required", "source": "Bullhorn DB"},
            {"canonical": "clearance", "alias": "detected_clearance", "source": "API v2"},
            {"canonical": "clearance", "alias": "Security Clearance", "source": "CSV"},
            {"canonical": "mapped_program", "alias": "program_name", "source": "Engine2"},
            {"canonical": "mapped_program", "alias": "Matched Program", "source": "CSV"},
            {"canonical": "bd_priority_score", "alias": "bd_score", "source": "Engine5"},
            {"canonical": "bd_priority_score", "alias": "BD Priority Score", "source": "CSV/Notion"},
        ],
    }
...
def _entity_activity():
    return {
        ...
        "properties": [
            {"name": "id", "type": "string", "source": "qdrant", "note": "Format: activity_{row_id}"},
            ...
            {"name": "content", "type": "text", "source": "qdrant_embedding", "note": "From comments"},
            {"name": "subject", "type": "string", "source": "qdrant", "note": "From note_text"},
            {"name": "date", "type": "datetime", "source": "bullhorn", "note": "activity_date"},
...
def _entity_prime_contractor():
    return {
        ...
        "storage": ["sqlite:bullhorn_master.db/prime_contractors", "qdrant:primes", "neo4j:Company"],
        "properties": [
            {"name": "id", "type": "integer", "source": "sqlite"},
            ...
            {"name": "relationship_status", "type": "string", "source": "manual", "values": ["Active Partner", "Prospect", "Target", "Watch", "Inactive"]},
...
def _entity_document():
    return {
        ...
        "properties": [
            ...
            {"name": "type", "type": "string", "source": "classifier", "example": "past_performance"},
            {"name": "_indexed_at", "type": "string", "source": "qdrant_meta"},
            {"name": "_embedding_model", "type": "string", "source": "qdrant_meta"},
        ],
        "aliases": [],
...
def _entity_contract_opportunity():
    return {
        ...
        "properties": [
            ...
            {"name": "type", "type": "string", "source": "sam_gov", "values": ["RFI", "RFP", "Sources Sought"]},
...
def _entity_memory():
    return {
        ...
        "properties": [
            {"name": "id", "type": "string", "source": "uuid"},
            {"name": "content", "type": "text", "source": "agent"},
            {"name": "memory_type", "type": "string", "source": "agent", "values": ["episodic", "semantic", "procedural"]},
            {"name": "importance", "type": "float", "source": "agent"},
            {"name": "layer", "type": "string", "source": "agent", "values": ["working", "episodic", "semantic", "procedural", "strategic"]},
...
def _entity_insight():
    return {
        ...
        "properties": [
            {"name": "id", "type": "integer", "source": "auto"},
            {"name": "entity_type", "type": "string", "source": "agent", "values": ["company", "program", "contact", "contract"]},
            {"name": "entity_name", "type": "string", "source": "agent"},
            {"name": "insight", "type": "text", "source": "agent"},
            {"name": "source", "type": "string", "source": "agent"},
            {"name": "confidence", "type": "float", "source": "agent", "note": "0.0-1.0"},
            {"name": "timestamp", "type": "timestamp", "source": "system"},
        ],
        "aliases": [],
    }
...
def _entity_data_quality_log():
    return {
        ...
        "properties": [
            ...
            {"name": "issue_type", "type": "string", "source": "qa_agent", "values": ["duplicate", "missing_field", "stale", "invalid", "low_confidence"]},
            {"name": "issue_description", "type": "text", "source": "qa_agent"},
            {"name": "severity", "type": "string", "source": "qa_agent", "values": ["critical", "high", "medium", "low"]},
...
def _entity_alert():
    return {
        ...
        "properties": [
            ...
            {"name": "severity", "type": "string", "source": "qa", "values": ["info", "warning", "critical"]},
...
def _entity_swarm_task():
    return {
        ...
        "properties": [
            ...
            {"name": "status", "type": "string", "source": "system", "values": ["pending", "running", "completed", "failed"]},
...
def _entity_workflow_run():
    return {
        ...
        "properties": [
            ...
            {"name": "status", "type": "string", "source": "system", "values": ["pending", "running", "completed", "failed", "paused"]},
...
def _entity_contact_claim():
    return {
        ...
        "properties": [
            ...
            {"name": "status", "type": "string", "source": "system", "values": ["active", "contested", "transferred", "expired"]},
...
def _entity_proposal_artifact():
    return {
        ...
        "properties": [
            {"name": "id", "type": "string", "source": "uuid"},
            {"name": "artifact_type", "type": "string", "source": "input", "values": ["capability_statement", "compliance_matrix", "rate_card", "past_performance", "labor_category"]},
            {"name": "program", "type": "string", "source": "input"},
...
def _build_relationships() -> list:
    return [
        ...
        {"source": "CONTACT", "target": "PRIME_CONTRACTOR", "type": "WORKS_AT", "description": "Contact employed by prime", "cardinality": "many-to-one"},
        {"source": "CONTACT", "target": "PROGRAM", "type": "WORKS_ON", "description": "Contact works on federal program", "cardinality": "many-to-many"},
        {"source": "CONTACT", "target": "ACTIVITY", "type": "HAS_ACTIVITY", "description": "Contact has CRM activity records", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "PLACEMENT", "type": "HAS_PLACEMENT", "description": "Contact placed in a role", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "JOB", "type": "SUBMITTED_FOR", "description": "Contact submitted for job order", "cardinality": "many-to-many"},
        {"source": "CONTACT", "target": "MEMORY", "type": "HAS_MEMORY", "description": "Agent memories about contact", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "INTERACTION", "type": "CONTACTED_BY", "description": "Agent interaction record", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "BULLHORN_NOTE", "type": "REFERENCED_IN", "description": "Contact mentioned in CRM notes", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "CONTACT", "type": "REFERRED_BY", "description": "Referral network relationship", "cardinality": "many-to-many"},
        {"source": "CONTACT", "target": "CONTACT", "type": "MANAGES", "description": "Neo4j manager-report relationship", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "CONTACT", "type": "REPORTS_TO", "description": "Neo4j reporting chain", "cardinality": "many-to-one"},
        {"source": "CONTACT", "target": "PRIME_CONTRACTOR", "type": "PREVIOUSLY_AT", "description": "Historical employment via candidate_prime_history", "cardinality": "many-to-many"},
        {"source": "CONTACT", "target": "LOCATION", "type": "LOCATED_IN", "description": "Contact location hub", "cardinality": "many-to-one"},
        {"source": "CONTACT", "target": "CALL_BRIEFING", "type": "HAS_BRIEFING", "description": "Pre-call briefing generated", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "TRANSCRIPT_INTEL", "type": "HAS_TRANSCRIPT", "description": "Call transcript intelligence", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "RELATIONSHIP_SCORE", "type": "HAS_SCORE", "description": "Relationship strength score", "cardinality": "one-to-many"},
        {"source": "CONTACT", "target": "CONTACT_CLAIM", "type": "CLAIMED_BY", "description": "BD team ownership claim", "cardinality": "one-to-many"},
        # PRIME_CONTRACTOR relationships
        {"source": "PRIME_CONTRACTOR", "target": "PROGRAM", "type": "PRIMES_ON", "description": "Contractor is prime on program", "cardinality": "one-to-many"},
        {"source": "PRIME_CONTRACTOR", "target": "PROGRAM", "type": "SUBS_TO", "description": "Contractor is sub on program", "cardinality": "many-to-many"},
        {"source": "PRIME_CONTRACTOR", "target": "JOB", "type": "POSTED_JOB", "description": "Prime posted job order", "cardinality": "one-to-many"},
        {"source": "PRIME_CONTRACTOR", "target": "CONTACT", "type": "EMPLOYS", "description": "Prime employs contact", "cardinality": "one-to-many"},
        {"source": "PRIME_CONTRACTOR", "target": "PAST_PERFORMANCE", "type": "HAS_PERFORMANCE", "description": "Prime has past performance record", "cardinality": "one-to-many"},
        {"source": "PRIME_CONTRACTOR", "target": "PRIME_CONTRACTOR", "type": "COMPETES_WITH", "description": "Competitor relationship", "cardinality": "many-to-many"},
        {"source": "PRIME_CONTRACTOR", "target": "PRIME_CONTRACTOR", "type": "TEAMS_WITH", "description": "Teaming partner relationship", "cardinality": "many-to-many"},
        {"source": "PRIME_CONTRACTOR", "target": "LOCATION", "type": "COMPANY_LOCATED_IN", "description": "Company facility location", "cardinality": "many-to-many"},
        # PROGRAM relationships
        {"source": "PROGRAM", "target": "JOB", "type": "HAS_JOB", "description": "Program has associated job postings", "cardinality": "one-to-many"},
        {"source": "PROGRAM", "target": "CONTACT", "type": "HAS_CONTACT", "description": "Program has associated contacts", "cardinality": "one-to-many"},
        {"source": "PROGRAM", "target": "DOCUMENT", "type": "HAS_DOCUMENT", "description": "Program has associated documents", "cardinality": "one-to-many"},
        {"source": "PROGRAM", "target": "PAST_PERFORMANCE", "type": "HAS_PERFORMANCE", "description": "Program has past performance data", "cardinality": "one-to-many"},
        {"source": "PROGRAM", "target": "CONTRACT_OPPORTUNITY", "type": "HAS_OPPORTUNITY", "description": "Program generates procurement opportunity", "cardinality": "one-to-many"},
        {"source": "PROGRAM", "target": "FEDERAL_CONTRACT", "type": "HAS_CONTRACT", "description": "Program has federal contract(s)", "cardinality": "one-to-many"},
        {"source": "PROGRAM", "target": "LOCATION", "type": "LOCATED_AT", "description": "Program execution location", "cardinality": "many-to-many"},
        {"source": "PROGRAM", "target": "RECOMPETE_PREDICTION", "type": "HAS_PREDICTION", "description": "Recompete timeline prediction", "cardinality": "one-to-many"},
        # JOB relationships
        {"source": "JOB", "target": "PLACEMENT", "type": "FILLED_BY", "description": "Job filled by placement", "cardinality": "one-to-many"},
        {"source": "JOB", "target": "PROGRAM", "type": "MAPPED_TO", "description": "Job mapped to program by Engine2", "cardinality": "many-to-one"},
        {"source": "JOB", "target": "PRIME_CONTRACTOR", "type": "POSTED_BY", "description": "Job posted by prime", "cardinality": "many-to-one"},
        {"source": "JOB", "target": "ACTIVITY", "type": "HAS_ACTIVITY", "description": "Job has associated CRM activities", "cardinality": "one-to-many"},
        {"source": "JOB", "target": "WIN_PROBABILITY", "type": "HAS_WIN_PROB", "description": "ML win probability prediction", "cardinality": "one-to-one"},
        # PLACEMENT relationships
        {"source": "PLACEMENT", "target": "JOB", "type": "FOR_JOB", "description": "Placement for specific job", "cardinality": "many-to-one"},
        {"source": "PLACEMENT", "target": "CONTACT", "type": "OF_CANDIDATE", "description": "Placement of candidate", "cardinality": "many-to-one"},
        {"source": "PLACEMENT", "target": "PRIME_CONTRACTOR", "type": "AT_PRIME", "description": "Placement at prime site", "cardinality": "many-to-one"},
        # ACTIVITY relationships
        {"source": "ACTIVITY", "target": "JOB", "type": "REGARDING_JOB", "description": "Activity about job", "cardinality": "many-to-one"},
        {"source": "ACTIVITY", "target": "CONTACT", "type": "REGARDING_CONTACT", "description": "Activity about contact", "cardinality": "many-to-one"},
        # DOCUMENT relationships
        {"source": "DOCUMENT", "target": "PROGRAM", "type": "ABOUT_PROGRAM", "description": "Document references program", "cardinality": "many-to-many"},
        {"source": "DOCUMENT", "target": "PRIME_CONTRACTOR", "type": "ABOUT_CONTRACTOR", "description": "Document references contractor", "cardinality": "many-to-many"},
        # INTELLIGENCE_REPORT relationships
        {"source": "INTELLIGENCE_REPORT", "target": "PROGRAM", "type": "ANALYZES", "description": "Report analyzes program", "cardinality": "many-to-many"},
        {"source": "INTELLIGENCE_REPORT", "target": "PRIME_CONTRACTOR", "type": "COVERS", "description": "Report covers contractor", "cardinality": "many-to-many"},
        # FEDERAL_CONTRACT relationships
        {"source": "FEDERAL_CONTRACT", "target": "PRIME_CONTRACTOR", "type": "AWARDED_TO", "description": "Contract awarded to prime", "cardinality": "many-to-one"},
        {"source": "FEDERAL_CONTRACT", "target": "PROGRAM", "type": "COVERS", "description": "Contract covers program", "cardinality": "many-to-many"},
        # INSIGHT relationships
        {"source": "INSIGHT", "target": "GRAPH_ENTITY", "type": "ABOUT_ENTITY", "description": "Insight relates to graph entity", "cardinality": "many-to-one"},
        # SOURCE_FILE relationships
        {"source": "SOURCE_FILE", "target": "PROCESSING_STATS", "type": "HAS_STATS", "description": "Source file has processing stats", "cardinality": "one-to-many"},
        {"source": "SOURCE_FILE", "target": "DATA_QUALITY_LOG", "type": "HAS_ISSUES", "description": "Source file has quality issues", "cardinality": "one-to-many"},
        # GRAPH relationships
        {"source": "GRAPH_ENTITY", "target": "GRAPH_ENTITY", "type": "GRAPH_RELATIONSHIP", "description": "LightRAG knowledge graph edge", "cardinality": "many-to-many"},
        # FILE_NODE relationships
        {"source": "FILE_NODE", "target": "FILE_NODE", "type": "DERIVED_FROM", "description": "Data lineage: output derived from input", "cardinality": "many-to-many"},
        {"source": "FILE_NODE", "target": "FILE_NODE", "type": "DEPENDS_ON", "description": "Code dependency", "cardinality": "many-to-many"},
        {"source": "PROCESS_NODE", "target": "FILE_NODE", "type": "READS", "description": "Process reads file", "cardinality": "many-to-many"},
        {"source": "PROCESS_NODE", "target": "FILE_NODE", "type": "WRITES", "description": "Process writes file", "cardinality": "many-to-many"},
        # CONTRACT_WATCH relationships
        {"source": "CONTRACT_WATCH", "target": "FEDERAL_CONTRACT", "type": "MONITORS", "description": "Watch monitors contracts", "cardinality": "many-to-many"},
        {"source": "CONTRACT_WATCH", "target": "CONTRACT_OPPORTUNITY", "type": "MONITORS", "description": "Watch monitors opportunities", "cardinality": "many-to-many"},
        # SWARM/WORKFLOW relationships
        {"source": "SWARM_TASK", "target": "WORKFLOW_RUN", "type": "EXECUTES_VIA", "description": "Swarm uses workflow", "cardinality": "one-to-many"},
        # STRATEGIC relationships
        {"source": "STRATEGIC_PATTERN", "target": "PROGRAM", "type": "ABOUT_PROGRAM", "description": "Pattern detected for program", "cardinality": "many-to-one"},
        {"source": "PROPOSAL_ARTIFACT", "target": "PROGRAM", "type": "FOR_PROGRAM", "description": "Artifact for program bid", "cardinality": "many-to-one"},
        # REVENUE relationships
        {"source": "REVENUE_PLACEMENT", "target": "PROGRAM", "type": "ON_PROGRAM", "description": "Revenue placement on program", "cardinality": "many-to-one"},
        {"source": "REVENUE_PLACEMENT", "target": "CONTACT", "type": "OF_CONTRACTOR", "description": "Revenue placement of contractor", "cardinality": "many-to-one"},
        # FORECAST relationships
        {"source": "HIRING_FORECAST", "target": "PROGRAM", "type": "FORECASTS_FOR", "description": "Hiring forecast for program", "cardinality": "many-to-one"},
    ]
...
def _build_data_flows() -> list:
    return [
        ...
        },
        {
            ...
            "transforms": ["parse", "standardize_18_fields", "match_programs", "score_bd_priority", "generate_playbooks", "export_notion_csv"],
            "trigger": "manual",
        ...
        },
        {
            ...
            "transforms": ["regex_tier_match", "location_hub_map", "outreach_sequence_assign"],
            "trigger": "manual",
        ...
        },
        {
            ...
            "transforms": ["clearance_boost", "program_boost", "location_boost", "tier_multiplier", "confidence_weight"],
            "trigger": "manual",
        ...
        },
        {
            ...
            "transforms": ["parse_csv", "normalize_names", "deduplicate", "link_fk", "compute_past_performance"],
            "trigger": "manual",
            "entities_touched": ["CONTACT", "JOB", "PLACEMENT", "ACTIVITY", "PRIME_CONTRACTOR", "PROGRAM", "PAST_PERFORMANCE"],
        },
        {
            "name": "Qdrant Indexing (Contacts)",
            "source": "bullhorn_master.db/candidates (426K rows)",
            "target": "qdrant:contacts (7,337 vectors)",
            "transforms": ["text_concat", "openai_embed_3_small", "uuid5_id", "classify_tier"],
        ...
        },
        {
            ...
            "transforms": ["search_awards", "search_opportunities", "match_watches", "generate_alerts"],
            "trigger": "manual | cron",
            "entities_touched": ["FEDERAL_CONTRACT", "CONTRACT_OPPORTUNITY", "CONTRACT_WATCH", "ALERT"],
        },
        {
            "name": "LightRAG Graph Building",
            "source": "All Qdrant collections",
            "target": "bd_graph.db (entities + relationships)",
            "transforms": ["entity_extraction", "relationship_extraction", "graph_build"],
            "trigger": "manual",
        ...
        },
        {
            ...
            "transforms": ["extract_nodes", "extract_relationships", "merge_nodes", "create_edges"],
            "trigger": "manual (POST /neo4j/ingest/*)",
            "entities_touched": ["CONTACT", "PROGRAM", "JOB", "PRIME_CONTRACTOR", "LOCATION"],
        },
        {
            ...
            "entities_touched": ["CONTACT", "JOB", "PROGRAM", "PLACEMENT", "PAST_PERFORMANCE", "PRIME_CONTRACTOR"],
        },
        {
        ...
        },
        {
            ...
            "transforms": ["ingest", "filter_relevance", "detect_recompete", "detect_competitor", "alert"],
            "trigger": "continuous (Pathway)",
            "entities_touched": ["CONTRACT_OPPORTUNITY", "FEDERAL_CONTRACT", "ACTIVITY", "ALERT", "STREAMING_EVENT"],
        },
        {
            "name": "Memory Consolidation",
            "source": "Episodic memories",
            "target": "Semantic facts + procedural insights",
            "transforms": ["scan_episodes", "extract_facts", "update_semantic", "generate_insights"],
        ...
        },
        {
            ...
            "transforms": ["aggregate_pipeline", "analyze_outreach", "scan_competitive", "generate_summary"],
            "trigger": "scheduled | manual (POST /reports/weekly)",
            "entities_touched": ["WEEKLY_REPORT", "CONTACT", "JOB", "PROGRAM", "ACTIVITY"],
        },
    ]
...
def _build_databases() -> dict:
    return {
        "qdrant_collections": [
            {"name": "contacts", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": 7337, "search_type": "dense", "payload_fields": ["name", "first_name", "last_name", "title", "company", "program", "email", "phone", "tier", "bd_priority", "clearance", "city", "state", "location_hub", "status", "skills", "notes", "source_db", "_indexed_at", "_embedding_model"]},
            {"name": "programs", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": 401, "search_type": "dense", "payload_fields": ["name", "acronym", "agency", "sub_agency", "prime_contractor", "location", "contract_value", "description", "mission_area", "contract_vehicle", "bd_priority", "status", "_indexed_at", "_embedding_model"]},
            {"name": "jobs", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": 4, "search_type": "dense", "payload_fields": ["title", "company", "location", "clearance", "description", "mapped_program", "bd_priority_score", "pipeline_stage", "status", "skills", "_indexed_at", "_embedding_model"]},
            {"name": "documents", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": 205, "search_type": "dense", "payload_fields": ["filename", "filepath", "doc_type", "title", "summary", "tags", "type", "chunk_index", "total_chunks", "_indexed_at", "_embedding_model"]},
            {"name": "activities", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": 500, "search_type": "dense", "payload_fields": ["activity_type", "content", "subject", "date", "contact_id", "job_id", "action", "source_db", "_indexed_at", "_embedding_model"]},
            {"name": "primes", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": 41, "search_type": "dense", "payload_fields": ["name", "aliases", "headquarters", "naics_codes", "contract_vehicles", "total_jobs", "total_placements", "total_revenue", "category", "_indexed_at", "_embedding_model"]},
            {"name": "bullhorn_notes", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": "50000+", "search_type": "hybrid", "payload_fields": ["note_body", "comments", "about", "action", "note_type", "personReference", "_source"]},
            {"name": "federal_contracts", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": "unknown", "search_type": "hybrid", "payload_fields": ["title", "description", "agency", "contractor", "contract_vehicle", "status"]},
            {"name": "intelligence_reports", "vector_size": 1536, "distance": "Cosine", "embedding_model": "text-embedding-3-small", "record_count": "unknown", "search_type": "hybrid", "payload_fields": ["content", "title", "summary", "source", "report_type", "classification", "date"]},
        ],
        "neo4j_graph": {
            "node_types": [
                {"type": "Person", "properties": ["name", "title", "tier", "bd_priority", "email", "phone", "linkedin", "location_hub", "functional_area", "program", "company", "source_db", "first_name", "last_name"]},
                {"type": "Company", "properties": ["name", "type", "revenue", "employee_count", "headquarters", "is_defense_prime", "website"]},
                {"type": "Program", "properties": ["name", "acronym", "value", "agency_owner", "prime_contractor", "pop_start", "pop_end", "clearance_req", "program_type", "confidence_level", "contract_vehicle", "hiring_velocity", "recompete_date", "naics"]},
                {"type": "Job", "properties": ["title", "status", "pay_rate", "bill_rate", "clearance", "date_added", "employment_type", "location", "source_url", "bd_priority", "functional_area"]},
                {"type": "Contract", "properties": ["vehicle", "value", "naics", "set_aside", "pop_start", "pop_end", "award_date", "contract_number"]},
                {"type": "Location", "properties": ["city", "state", "coordinates_lat", "coordinates_lon", "hub_name", "military_installation", "region"]},
                {"type": "Interaction", "properties": ["date", "type", "summary", "sentiment", "author", "action", "status", "note_body"]},
                {"type": "File", "properties": ["path", "name", "type", "extension", "size_bytes", "hash", "modified", "engine", "is_input", "is_output"]},
                {"type": "Process", "properties": ["name", "script_path", "engine", "description", "last_run", "run_count", "avg_duration_seconds"]},
            ],
            "relationship_types": [
                "WORKS_AT", "MANAGES", "REPORTS_TO", "LOCATED_IN", "CONTACTED_BY",
                "PRIMES_ON", "SUBS_TO", "COMPETES_WITH", "COMPANY_LOCATED_IN",
                "OWNED_BY", "LOCATED_AT", "HAS_CONTRACT", "REQUIRES_CLEARANCE",
                "POSTED_BY", "MAPPED_TO", "JOB_AT", "REQUIRES_SKILL",
                "AWARDED_TO", "COVERS",
                "BETWEEN", "ABOUT", "BY_USER",
                "DERIVED_FROM", "DEPENDS_ON", "READS", "WRITES",
            ],
        },
        "sqlite_databases": [
            {
                ...
                "size_mb": 293,
                "tables": ["jobs", "candidates", "placements", "activities", "prime_contractors", "programs", "past_performance", "job_program_mapping", "job_prime_mapping", "candidate_prime_history", "source_files", "data_quality_log", "processing_stats"],
            },
            {"name": "bd_graph.db", "path": "Engine8_Knowledge/data/bd_graph.db", "size_mb": 0.8, "tables": ["entities", "relationships"]},
            {"name": "memories.db", "path": "Engine8_Knowledge/data/memories.db", "size_mb": 0.04, "tables": ["memories", "interactions", "insights"]},
            {"name": "page_index.db", "path": "Engine8_Knowledge/data/page_index.db", "size_mb": 0.01, "tables": ["pages"]},
            {"name": "checkpoints_meta.db", "path": "Engine8_Knowledge/data/checkpoints_meta.db", "size_mb": 0.01, "tables": ["threads", "checkpoint_snapshots"]},
            {"name": "langgraph_checkpoints.db", "path": "Engine8_Knowledge/data/langgraph_checkpoints.db", "size_mb": 0.01, "tables": ["workflow_metadata"]},
        ],
        "notion_databases": [
            {"name": "Program Mapping Intelligence Hub", "id": "0a0d7e46-3d88-40b6-853a-3c9680347644", "properties": ["Job Title", "Company", "Location", "Clearance", "Matched Program", "BD Score", "Priority Tier", "Employment Type", "Outreach Status", "Duration", "BD Formula Message", "PTS Available Contractors"]},
            {"name": "Federal Programs", "id": "9db40fce-0781-42b9-902c-d4b0263b1e23", "properties": ["Program Name", "Acronym", "Agency", "Prime Contractor", "Contract Value", "BD Priority", "Hiring Velocity", "Mission Area", "BD Approach Notes", "Next Actions"]},
            {"name": "Contractors Database", "id": "ca67175b-df3d-442d-a2e7-cc24e9a1bf78", "properties": ["Name", "Relationship Status", "PTS Placements Made", "Active Placements", "Portfolio Value", "Last Engagement Date"]},
            {"name": "DCGS Contacts Full", "id": "2ccdef65-baa5-80d0-9b66-c67d66e7a54d", "properties": ["Name", "Title", "Company", "Program", "Clearance", "Hierarchy Tier", "BD Priority", "Location Hub", "Last Contact Date", "Outreach History", "Relationship Strength"]},
            {"name": "Contract Vehicles Master", "id": "e1166305-1b1f-4812-b665-bcfa6a87a2ab", "properties": ["Vehicle Name", "Contract Number", "Agency", "Prime Contractor", "Value", "Period"]},
            {"name": "GDIT Other Contacts", "id": "c1b1d358-9d82-4f03-b77c-db43d9795c6f", "properties": ["Name", "Title", "Company", "Program", "Contact Info"]},
        ],
        "api_endpoints": {
            "total_estimated": 340,
            "core_api": {"file": "Engine8_Knowledge/api.py", "prefix": "/", "endpoints": 67},
            "unified_v2": {"file": "api/unified_endpoints.py", "prefix": "/api/v2", "endpoints": 13},
            "engine8_sub_routers": [
            ...
            "src_api_routers": [
                {"file": "src/api/knowledge_api.py", "phase": "39A", "endpoints": 15, "focus": "Temporal knowledge, entity resolution"},
                {"file": "src/api/relationship_api.py", "phase": "34A", "endpoints": 14, "focus": "Relationship strength, influence scoring"},
                {"file": "src/api/proposal_api.py", "phase": "35A", "endpoints": 10, "focus": "Proposal artifacts, compliance matrices"},
                {"file": "src/api/revenue_api.py", "phase": "36A", "endpoints": 16, "focus": "Revenue tracking, margin analysis, ROI"},
                {"file": "src/api/tenant_api.py", "phase": "37A", "endpoints": 21, "focus": "Multi-tenancy, auth, RBAC"},
                {"file": "src/api/data_quality_api.py", "phase": "38A", "endpoints": 17, "focus": "Quality scoring, self-healing, lineage"},
                {"file": "src/api/rag_api.py", "phase": "40A", "endpoints": 10, "focus": "Agentic RAG, query decomposition"},
                {"file": "src/api/swarm_api.py", "phase": "41A", "endpoints": 10, "focus": "Multi-agent swarms, DAG execution"},
                {"file": "src/api/memory_api.py", "phase": "42A", "endpoints": 12, "focus": "5-layer memory, consolidation"},
                {"file": "src/api/governance_api.py", "phase": "43A", "endpoints": 12, "focus": "Data catalog, schema registry, SLAs"},
                {"file": "src/api/intelligence_api.py", "phase": "44A", "endpoints": 13, "focus": "Meta-learning, strategic patterns"},
                {"file": "src/api/mcp_api.py", "phase": "45A", "endpoints": 10, "focus": "MCP ecosystem orchestration"},
                {"file": "src/api/voice_api.py", "phase": "46A", "endpoints": 12, "focus": "Call briefings, transcript intel"},
                {"file": "src/api/embeddings_api.py", "phase": "47A", "endpoints": 12, "focus": "Fine-tuning, benchmarking, A/B tests"},
                {"file": "src/api/geo_api.py", "phase": "48A", "endpoints": 10, "focus": "Geocoding, spatial queries, clusters"},
                {"file": "src/api/workflows_api.py", "phase": "49A", "endpoints": 12, "focus": "Temporal workflows, task queues"},
                {"file": "src/api/collaboration_api.py", "phase": "50A", "endpoints": 12, "focus": "CRDT rooms, contact claiming, intel feed"},
                {"file": "src/api/simulation_api.py", "phase": "51A", "endpoints": 14, "focus": "Causal inference, digital twins"},
                {"file": "src/api/security_api.py", "phase": "52A", "endpoints": 14, "focus": "ABAC, audit trail, field encryption"},
                {"file": "src/api/observability_api.py", "phase": "53A", "endpoints": 12, "focus": "Tracing, metrics, SLOs"},
                {"file": "src/api/experimentation_api.py", "phase": "55A", "endpoints": 12, "focus": "Feature flags, A/B testing"},
                {"file": "src/api/resilience_api.py", "phase": "54A", "endpoints": 12, "focus": "Circuit breakers, chaos engineering"},
                {"file": "src/api/scaling_api.py", "phase": "57A", "endpoints": 12, "focus": "Connection pools, replicas, auto-scaling"},
                {"file": "src/api/pwa_api.py", "phase": "58A", "endpoints": 10, "focus": "PWA, push notifications, offline sync"},
                {"file": "src/api/streaming_api.py", "phase": "31A", "endpoints": 14, "focus": "Event bus, WebSocket, workflows"},
                {"file": "src/api/predictive_api.py", "phase": "32A", "endpoints": 14, "focus": "Win probability, hiring forecasts"},
                {"file": "src/api/nlq_api.py", "phase": "33A", "endpoints": 9, "focus": "Natural language queries, autocomplete"},
            ],
        },
        "csv_sources": [
            {"path": "engine_data/Engine2_ProgramMapping/Federal Programs MASTER V4.csv", "columns": ["Program Name", "Acronym", "Agency", "Prime Contractor", "Contract Number", "Contract Value", "Contract Description", "Match Confidence", "Match Score"]},
            {"path": "engine_data/Engine3_OrgChart/Contact_Search_List_MASTER.csv", "columns": ["Job Source", "Job Number", "Job Title", "Job Location", "Security Clearance", "Matched Programs", "Prime Contractors", "Program Locations", "Target Titles (Bullhorn)", "Search Keywords (ZoomInfo)", "LinkedIn Search", "Match Score", "Match Reasons"]},
            {"path": "engine_data/Engine3_OrgChart/Contacts_TEMPLATE.csv", "columns": ["Name", "Job Title", "Email Address", "Phone", "LinkedIn URL", "Company", "Program", "Location", "Clearance", "Hierarchy Tier", "BD Priority", "Location Hub", "Functional Area", "Last Contact Date", "Notes", "Source"]},
            {"path": "engine_data/Engine2_ProgramMapping/Programs_KB_TEMPLATE.csv", "columns": ["Program Name", "Acronym", "Agency Owner", "Prime Contractor", "Known Subcontractors", "Contract Value", "Contract Vehicle", "Key Locations", "Clearance Requirements", "Typical Roles", "Keywords", "PTS Involvement", "Priority Level", "Pain Points", "Notes"]},
            {"path": "engine_data/Engine1_Scraper/Jobs_Mapped_to_Programs_MASTER.csv", "columns": ["Job Source", "Job Number", "Job Title", "Job Location", "Security Clearance", "Employment Type", "Date Posted", "Matched Program", "Program Acronym", "Program Agency", "Prime Contractor", "Program Location", "Contract Number", "Match Score", "Match Confidence", "Match Reasons", "Job URL"]},
            {"path": "Engine7_BullhornETL/colton_scurry_analysis/master_notes.csv", "columns": ["source_sheet", "department", "note_author", "date_added", "type", "action", "about", "status", "note_body_raw", "note_body_clean", "extracted_primes", "extracted_programs", "extracted_contracts", "extracted_roles", "extracted_locations", "extracted_headcount", "extracted_bill_rates", "extracted_dates_mentioned", "extracted_experience_levels", "extracted_skills", "extracted_acronyms", "extracted_emails", "extracted_phone_numbers"]},
            {"path": "docs/N8N-Builder.Capture-MCP-server/bd_targets.csv", "columns": ["Program Name", "Acronym", "Agency", "BD Priority", "Prime Contractor", "Contract Value", "Recompete Date", "Period of Performance", "Key Locations", "Contract Number"]},
            {"path": "docs/N8N-Builder.Capture-MCP-server/subaward_intelligence.csv", "columns": ["Program Name", "Acronym", "Agency", "Prime Contractor", "Contract Number", "Contract Value", "Subaward Count", "Subaward Total", "Recipient UEI", "Recipient Name", "City", "State"]},
        ],
        "streaming_schemas": [
            {"name": "SAMOpportunitySchema", "source": "SAM.gov", "fields": ["notice_id", "title", "agency", "posted_date", "response_deadline", "set_aside", "naics_code", "description", "estimated_value", "place_of_performance"]},
            {"name": "FPDSContractSchema", "source": "FPDS", "fields": ["contract_id", "vendor_name", "agency", "award_date", "award_amount", "pop_start_date", "pop_end_date", "naics_code", "description", "contract_type"]},
            {"name": "BullhornActivitySchema", "source": "Bullhorn", "fields": ["activity_id", "contact_id", "activity_type", "timestamp", "notes", "outcome"]},
            {"name": "AlertSchema", "source": "Generated", "fields": ["alert_id", "alert_type", "title", "description", "priority", "source_id", "timestamp", "metadata"]},
        ],
    }
...
def _build_engines() -> list:
    return [
        {"id": "engine1", "name": "Apify Job Scraper", "path": "Engine1_Scraper/", "status": "configured"},
        {"id": "engine2", "name": "Program Mapping", "path": "Engine2_ProgramMapping/", "status": "complete", "key_scripts": ["pipeline.py", "job_standardizer.py", "program_mapper.py", "exporters.py"]},
        {"id": "engine3", "name": "OrgChart Contact Classification", "path": "Engine3_OrgChart/", "status": "complete", "key_scripts": ["contact_classifier.py"]},
        {"id": "engine4", "name": "BD Playbook Generator", "path": "Engine4_Playbook/", "status": "complete", "key_scripts": ["bd_playbook_generator.py"]},
        {"id": "engine5", "name": "BD Priority Scoring", "path": "Engine5_Scoring/", "status": "complete", "key_scripts": ["bd_scoring.py"]},
        {"id": "engine6", "name": "QA & Alerts", "path": "Engine6_QA/", "status": "in_progress"},
        {"id": "engine7", "name": "Bullhorn ETL", "path": "Engine7_BullhornETL/", "status": "complete", "database": "bullhorn_master.db (293 MB, 13 tables)"},
        {"id": "engine8", "name": "AI Knowledge System", "path": "Engine8_Knowledge/", "status": "complete", "server": "http://localhost:8100", "endpoints": 340, "collections": 9, "agents": 13},
    ]
