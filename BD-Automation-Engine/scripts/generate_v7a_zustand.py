#!/usr/bin/env python3
"""Generate PTS Data Architecture Explorer V7A — Zustand store slices + Atomic component architecture."""
import json
import os
...
/* Focus mode — now driven via inline styles on ReactFlow wrapper */
.focus-active .react-flow__node.dimmed{opacity:.12!important;filter:saturate(0.2)!important;transition:opacity .3s,filter .3s}
.focus-active .react-flow__edge.dimmed path{stroke:rgba(148,163,184,0.04)!important;transition:stroke .3s}
.focus-active .react-flow__edge.dimmed .react-flow__edge-text{fill:transparent!important}
.focus-active .react-flow__node.highlighted{opacity:1!important;filter:none!important;transition:opacity .3s}
.focus-active .react-flow__edge.highlighted path{stroke:rgba(34,211,238,0.5)!important;stroke-width:2!important;transition:stroke .3s}
.focus-active .react-flow__edge.highlighted .react-flow__edge-text{fill:var(--cyan)!important}
.focus-active .react-flow__edge.highlighted .react-flow__edge-textbg{fill:var(--bg-0)!important;fill-opacity:0.9!important}

/* Search highlight */
.search-active .react-flow__node.search-match{box-shadow:0 0 0 2px var(--cyan),0 0 20px rgba(34,211,238,.25)!important;border-radius:5px}
.search-active .react-flow__node:not(.search-match){opacity:.2!important;filter:saturate(0.15)!important;transition:opacity .3s,filter .3s}
.search-active .react-flow__edge path{stroke:rgba(148,163,184,0.05)!important}
...
// ─── Pre-computed module-level adjacency maps ───
const ADJ = {};
...
// ─── Zustand Store ───
// Combines four logical slices: Graph, Selection, Search, UI
const useStore = create((set, get) => ({
  ...
  toggleCategory: (cat) => set(state => {
    if (cat === '__ALL__') {
      const allOn = state.activeCategories.size === ALL_CATEGORY_KEYS.length;
      return { activeCategories: allOn ? new Set() : new Set(ALL_CATEGORY_KEYS) };
    }
  ...
  navigateToEntity: (entityId) => {
    ...
    set(state => {
      ...
      return { selectedNode: node.data, focusId: entityId, navHistory: newHistory };
    });
  },
  ...
  clearSelection: () => set({ selectedNode: null, focusId: null, navHistory: [] }),
  ...
  // === UI Slice ===
  tab: 'graph',
  setTab: (tab) => set({ tab, selectedNode: null, focusId: null, navHistory: [], searchQuery: '' }),
  hoverData: null,
  hoverPos: null,
...
function CategoryDot({ color, off }) {
  return html`<div className="cat-toggle-dot" style=${{ background: color, opacity: off ? 0.15 : 1 }}></div>`;
}

function Badge({ children, style }) {
  return html`<span className="record-badge" style=${style}>${children}</span>`;
}

function StatValue({ value, color, fontSize }) {
  return html`<span className="stat-value" style=${{ color: color || 'var(--cyan)', fontSize: fontSize || '16px' }}>${value}</span>`;
}

function SearchInput({ value, onChange, placeholder }) {
  return html`<input className="search-input" placeholder=${placeholder || 'Search entities\u2026'} value=${value} onInput=${e => onChange(e.target.value)} />`;
}

function ToolbarButton({ label, active, onClick, style }) {
  return html`<button className=${'toolbar-btn' + (active ? ' active' : '')} onClick=${onClick} style=${style}>${label}</button>`;
}

function KeyboardHint({ keyLabel, description }) {
  return html`<span><span className="kb-key">${keyLabel}</span> ${description}</span>`;
}


// ─── Molecules ───

// Entity node for the graph view — reads focusId and searchQuery from store for className computation
  ...
  return html`
    <div className=${'entity-node' + (selected ? ' selected' : '')} style=${{ borderLeft: '3px solid ' + c }}>
      ...
      <div className="entity-header" style=${{ background: c + '15' }}>
        ...
        ${data.records && data.records !== '\u2014' && html`<${Badge}>${data.records}</${Badge}>`}
      </div>
...
const FlowNode = memo(FlowNodeComponent, (prev, next) => prev.data === next.data);
...
const IntegNode = memo(IntegNodeComponent, (prev, next) => prev.data === next.data);

function SearchResultItem({ node, onClick }) {
  return html`<div className="search-result-item" onClick=${onClick}>
    <div className="sr-dot" style=${{ background: node.data.color }}></div>
    <span className="sr-name">${node.data.label}</span>
    <span className="sr-cat">${node.data.category.split(' ')[0]}</span>
    <span className="sr-rels">${node.data.relCount}r</span>
  </div>`;
}

function RelationshipItem({ direction, targetId, onClick }) {
  const arrow = direction === 'outgoing' ? '\u2192' : '\u2190';
  return html`<div className="rel-item" style=${{ marginLeft: '8px', cursor: 'pointer' }} onClick=${onClick}>
    <span className="rel-arrow">${arrow}</span>
    <span className="rel-target" style=${{ borderBottom: '1px dotted var(--border-1)' }}>${ID_TO_LABEL[targetId] || targetId}</span>
  </div>`;
}

function NavBreadcrumb({ navHistory, onNavigate, onBack }) {
  if (navHistory.length <= 1) return null;
  return html`<div className="nav-history">
    ...
    ${navHistory.map((h, i) => html`<${React_Fragment} key=${i}>
      ...
      <span className=${'nav-crumb' + (i === navHistory.length - 1 ? ' current' : '')}
        onClick=${() => i < navHistory.length - 1 && onNavigate(h.id)}>${h.label}</span>
    </${React_Fragment}>`)}
  </div>`;
}
...
// === Entity Graph View ===
// Subscribes to: activeCategories, searchQuery, grouped, focusId from store
function EntityGraphView() {
  ...
  // React state-driven focus mode: apply className to nodes/edges
  useEffect(() => {
    if (!focusId) {
  ...
  // React state-driven search highlight
  useEffect(() => {
  ...
  const onPaneClick = useCallback(() => { setFocusId(null); }, [setFocusId]);
...
function DashboardView() {
  ...
  return html`<div className="dash-grid">
    ...
    <div className="dash-card"><h3>Storage</h3>
      <div className="stat-row"><span className="stat-label">Qdrant Collections</span><${StatValue} value=${d.qdrantCollections} color="#22d3ee" /></div>
      <div className="stat-row"><span className="stat-label">SQLite Databases</span><${StatValue} value=${d.sqliteDatabases} color="#f59e0b" /></div>
      <div className="stat-row"><span className="stat-label">Neo4j Node Types</span><${StatValue} value=${d.neo4jNodeTypes} color="#10b981" /></div>
      <div className="stat-row"><span className="stat-label">Neo4j Rel Types</span><${StatValue} value=${d.neo4jRelTypes} color="#10b981" /></div>
    </div>
    <div className="dash-card"><h3>Infrastructure</h3>
      <div className="stat-row"><span className="stat-label">API Endpoints</span><${StatValue} value=${d.apiEndpoints} color="#3b82f6" /></div>
      <div className="stat-row"><span className="stat-label">Data Flows</span><${StatValue} value=${d.dataFlows} color="#8b5cf6" /></div>
      <div className="stat-row"><span className="stat-label">Engines</span><${StatValue} value=${d.engines} color="#f43f5e" /></div>
      <div className="stat-row"><span className="stat-label">Data Coverage</span><${StatValue} value=${withRecords + '/' + d.totalEntities} color="#22d3ee" /></div>
    </div>
    <div className="dash-card"><h3>Most Connected Entities</h3>
      ${topRels.map(n => html`<div className="stat-row" key=${n.id}><span className="stat-label" style=${{ display: 'flex', alignItems: 'center', gap: '6px' }}><span style=${{ width: 6, height: 6, borderRadius: 2, background: n.data.color, display: 'inline-block' }}></span>${n.data.label}</span><${StatValue} value=${n.data.relCount} color=${n.data.color} /></div>`)}
    </div>
    <div className="dash-card"><h3>Top Relationship Types</h3>
      ${topRelTypes.map(([lbl, cnt]) => html`<div className="stat-row" key=${lbl}><span className="mono" style=${{ fontSize: '11px', color: 'var(--cyan)' }}>${lbl}</span><${StatValue} value=${cnt} color="var(--text-2)" fontSize="14px" /></div>`)}
    </div>
...
// === Detail Panel — reads selection state from store ===
function DetailPanel() {
  ...
  return html`<div className="detail-panel" ref=${panelRef}>
    <${NavBreadcrumb} navHistory=${navHistory} onNavigate=${navigateToEntity} onBack=${goBack} />
    <div className="detail-hdr">
      <div style=${{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
        <div>
          <div style=${{ fontSize: '16px', fontWeight: 600, marginBottom: '4px' }}>${data.label}</div>
          <div style=${{ display: 'flex', gap: '6px', alignItems: 'center' }}>
            <${CategoryDot} color=${data.color} />
        ...
        <button onClick=${clearSelection} style=${{ background: 'none', border: 'none', color: 'var(--text-3)', cursor: 'pointer', fontSize: '18px', lineHeight: 1 }}>${'\u00D7'}</button>
      </div>
    ...
    ${(outE.length > 0 || inE.length > 0) && html`<div className="detail-sec"><div className="detail-sec-title">Relationships (${outE.length + inE.length})</div>
      ${outGroups.length > 0 && html`<div className="rel-group">
        ...
        ${openGroups.has('outgoing') && outGroups.map(([lbl, targets]) => html`<div key=${lbl} style=${{ marginLeft: '12px', marginBottom: '6px' }}>
          ...
          ${targets.map(t => html`<${RelationshipItem} key=${t} direction="outgoing" targetId=${t} onClick=${() => navigateToEntity(t)} />`)}
        </div>`)}
      </div>`}
      ${inGroups.length > 0 && html`<div className="rel-group">
        ...
        ${openGroups.has('incoming') && inGroups.map(([lbl, sources]) => html`<div key=${lbl} style=${{ marginLeft: '12px', marginBottom: '6px' }}>
          ...
          ${sources.map(s => html`<${RelationshipItem} key=${s} direction="incoming" targetId=${s} onClick=${() => navigateToEntity(s)} />`)}
        </div>`)}
      </div>`}
    </div>`}
...
function CategorySidebar() {
  ...
  const handleSearchResultClick = useCallback(entityId => {
    navigateToEntity(entityId);
    setSearchQuery('');
  }, [navigateToEntity, setSearchQuery]);

  const allOn = activeCategories.size === ALL_CATEGORY_KEYS.length;
  ...
  return html`<div className="cat-sidebar">
    <div className="cat-sidebar-hdr">
      ...
      ${searchQuery && searchMatches.length > 0 && html`<div className="search-results">
        ${searchMatches.slice(0, 12).map(n => html`<${SearchResultItem} key=${n.id} node=${n} onClick=${() => handleSearchResultClick(n.id)} />`)}
      ...
      <div style=${{ display: 'flex', gap: '6px', marginTop: '8px' }}>
        <${ToolbarButton} label=${allOn ? 'Hide All' : 'Show All'} onClick=${() => toggleCategory('__ALL__')} />
        <${ToolbarButton} label="Grouped" active=${grouped} onClick=${toggleGrouped} />
      </div>
    </div>
    ${Object.entries(CATEGORIES).map(([n, i]) => html`<div className=${'cat-toggle' + (activeCategories.has(n) ? '' : ' off')} key=${n} onClick=${() => toggleCategory(n)}><${CategoryDot} color=${i.color} off=${!activeCategories.has(n)} /><span className="cat-toggle-label">${n}</span><span className="cat-toggle-count">${i.count}</span></div>`)}
  </div>`;
}

// === Hover Tooltip — reads from store ===
function HoverTooltip() {
  ...
  return html`<div className="hover-tooltip" style=${{ left: hoverPos.x + 12, top: hoverPos.y - 10 }}>
    <div className="ht-title">${hoverData.label}</div>
    <div className="ht-cat">${hoverData.category}</div>
    ${hoverData.description && html`<div className="ht-desc">${hoverData.description.length > 120 ? hoverData.description.slice(0, 120) + '\u2026' : hoverData.description}</div>`}
    <div className="ht-stats">
      ${hoverData.records && hoverData.records !== '\u2014' && html`<span>${hoverData.records} records</span>`}
...
// === Top Bar — reads tab and focusId from store ===
function TopBar() {
  ...
  return html`<div className="top-bar">
    <span className="top-bar-title">PTS Data Architecture</span>
    <span className="top-bar-ver">V7A</span>
    <div style=${{ width: 1, height: 20, background: 'var(--border-0)' }}></div>
    <div style=${{ display: 'flex', gap: 0 }}>${tabs.map(t => html`<button key=${t.id} className=${'tab-btn' + (tab === t.id ? ' active' : '')} onClick=${() => setTab(t.id)}>${t.label}</button>`)}</div>
    <div style=${{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '12px' }}>
      ...
      <${ToolbarButton} label=${'\u2B07 Export'} onClick=${exportArchData} style=${{ fontSize: '9px' }} />
...
function AppLayout() {
  ...
  return html`<div style=${{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
    ...
    <${HoverTooltip} />
    ${tab === 'graph' && html`<div className="kb-hint">
      <${KeyboardHint} keyLabel="Esc" description="Clear" />
      <${KeyboardHint} keyLabel="/" description="Search" />
      <${KeyboardHint} keyLabel="Click" description="Focus" />
      ${navHistory.length > 1 && html`<${KeyboardHint} keyLabel=${'\u232B'} description="Back" />`}
      <${KeyboardHint} keyLabel="Scroll" description="Zoom" />
    </div>`}
  </div>`;
}
...
<title>PTS Data Architecture Explorer V7A</title>
...
outpath = os.path.join(BASE, 'PTS_DATA_ARCHITECTURE_EXPLORER_V7A.html')
with open(outpath, 'w', encoding='utf-8') as f:
    f.write(output)

print(f"V7A written: {len(output):,} chars ({len(output)//1024}KB)")
