            ...
            for page in results:
                try:
                    ...
                    program = FederalProgram(
                        ...
                        subcontractors=self._extract_text(
                            props.get("Known Subcontractors")
                        ),
                        pts_involvement=self._extract_select(
                            props.get("PTS Involvement")
                        ),
                        ...
                        clearance_requirements=self._extract_multi_select(
                            props.get("Clearance Requirements")
                        ),
                    )
                    if program.name:
...
class PTSPastPerformanceMatcher:
    ...
    def find_past_performance(self, job: Dict) -> Dict:
        ...
        for prog in self.pts_programs:
            score = self._score_program_match(
                prog, job_prime, job_program, job_location
            )
            if score > 0:
    ...
    def _score_program_match(
        self, prog: FederalProgram, job_prime: str, job_program: str, job_location: str
    ) -> float:
...
def find_pts_past_performance_for_jobs(
    jobs: List[Dict], programs: List[FederalProgram] = None
) -> List[Dict]:
