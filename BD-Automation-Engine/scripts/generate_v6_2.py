#!/usr/bin/env python3
"""Generate PTS Data Architecture Explorer V6.2 — focus mode, swim lanes, polished."""
import json
import os
...
// === Entity Graph View with Focus Mode ===
function EntityGraphView({ onSelectNode, activeCategories, searchQuery, useGrouped, focusNodeId, onFocusChange }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      fn = fn.filter(n => n.data.label.toLowerCase().includes(q) || n.data.category.toLowerCase().includes(q) || (n.data.description && n.data.description.toLowerCase().includes(q)));
    }
...
// === Detail Panel with property notes ===
function DetailPanel({ data, onClose }) {
  ...
  return html`<div className="detail-panel">
    ...
    ${(outE.length > 0 || inE.length > 0) && html`<div className="detail-sec"><div className="detail-sec-title">Relationships (${outE.length + inE.length})</div>
      ${outE.length > 0 && html`<div style=${{ marginBottom: '8px' }}><div style=${{ fontSize: '9px', color: 'var(--text-3)', marginBottom: '4px', fontFamily: 'JetBrains Mono', letterSpacing: '.1em' }}>OUTGOING (${outE.length})</div>${outE.map(e => html`<div className="rel-item" key=${e.id}><span className="rel-arrow">${'\u2192'}</span><span className="rel-label">${e.label || (e.data && e.data.label) || ''}</span><span className="rel-target">${e.target}</span></div>`)}</div>`}
      ${inE.length > 0 && html`<div><div style=${{ fontSize: '9px', color: 'var(--text-3)', marginBottom: '4px', fontFamily: 'JetBrains Mono', letterSpacing: '.1em' }}>INCOMING (${inE.length})</div>${inE.map(e => html`<div className="rel-item" key=${e.id}><span className="rel-arrow">${'\u2190'}</span><span className="rel-label">${e.label || (e.data && e.data.label) || ''}</span><span className="rel-target">${e.source}</span></div>`)}</div>`}
    </div>`}
...
function App() {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.key === 'Escape') { setSel(null); setFocusId(null); }
    };
  ...
  return html`<div style=${{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
    <div className="top-bar">
      ...
      <span className="top-bar-ver">V6.2</span>
      ...
      <div style=${{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ${focusId && html`<span className="mono" style=${{ fontSize: '10px', color: 'var(--cyan)', background: 'rgba(34,211,238,.08)', padding: '2px 8px', borderRadius: '3px' }}>Focus: ${focusId}</span>`}
    ...
    <div style=${{ display: 'flex', flex: 1, overflow: 'hidden' }}>
      ...
      ${sel && tab === 'graph' && html`<${DetailPanel} data=${sel} onClose=${() => { setSel(null); setFocusId(null); }} />`}
    </div>
    ${tab === 'graph' && html`<div className="kb-hint"><span><span className="kb-key">Esc</span> Clear focus</span><span><span className="kb-key">Click</span> Focus node</span><span><span className="kb-key">Scroll</span> Zoom</span></div>`}
  </div>`;
}
...
<title>PTS Data Architecture Explorer V6.2</title>
...
outpath = os.path.join(BASE, 'PTS_DATA_ARCHITECTURE_EXPLORER_V6.2.html')
with open(outpath, 'w', encoding='utf-8') as f:
    f.write(output)

print(f"V6.2 written: {len(output):,} chars ({len(output)//1024}KB)")
