# BD Playbook: Senior Network Engineer - DCGS
## Distributed Common Ground System (AF) Opportunity

**Generated:** 2026-01-25 23:06
**Priority:** 🔥 Hot (Score: 95/100)
**Match Confidence:** 50% (fuzzy)

---

## 1. Program Intel

### Program Overview
**Program:** Distributed Common Ground System (AF)
**Customer Agency:** DoD/IC
**Prime Contractor:** Unknown
**Location:** San Diego, CA

### Opportunity Details
**Position:** Senior Network Engineer - DCGS
**Clearance Required:** TS/SCI
**Company:** None

### Match Signals
- Location match: San Diego -> AF DCGS - PACAF
- 'des' in description
- 'des' in description
- 'dcgs' in title
- 'ngi' in title
- 'nee' in title
- DCGS keyword: 'dcgs'

### Job Description
Support the Air Force Distributed Common Ground System (DCGS) program in a senior network engineering role. Responsible for designing, implementing, and maintaining network infrastructure supporting ISR (Intelligence, Surveillance, and Reconnaissance) operations. Work with cross-functional teams to ensure 24/7 network availability for critical Air Force intelligence systems.

---

## 2. Org Intel

### Key Contacts
No contacts found in database. Manual research recommended.

### Decision Makers
No decision makers identified. Target Tier 1-3 contacts.

---

## 3. Pain Points & Opportunities

### Identified Pain Points
- TS/SCI requirement reduces available talent by ~90%
- San Diego is understaffed for DCGS - critical hiring need

### Staffing Signals
- **Clearance Level:** TS/SCI (indicates high-value cleared position, competitive market)
- **Location:** San Diego, CA (market conditions: understaffed DCGS site, high demand)
- **Technologies:** Cisco, Juniper, VMware NSX, Linux

### Certification Requirements
- No specific certifications required

---

## 4. Competitive Landscape

### Current Landscape
**Prime Contractor:** Unknown
**Our Position:** Direct bid opportunity

### Win Themes
- **Cleared Talent Pipeline:** Emphasize active pool of high-clearance candidates
- **Pain Point Solutions:** Address identified staffing challenges directly
- **Local Presence:** Highlight San Diego, CA capabilities or remote flexibility

### Competitive Differentiators
- Active TS/SCI candidate pipeline (vs. starting from scratch)
- Past performance on DoD/IC programs
- Retention-focused approach (reduces customer risk)
- Rapid deployment capability (2-3 week typical)

---

## 5. Action Plan

### Immediate Actions
1. **IMMEDIATE:** Reach out to identified contacts within 24 hours
2. Research None BD/capture team contacts
3. Prepare Distributed Common Ground System (AF) capabilities one-pager
4. Track Distributed Common Ground System (AF) news and contract announcements
5. Update CRM with opportunity details

### Recommended Outreach Sequence
**Hot Priority Sequence:**
- Day 1: Send intro email to primary contact
- Day 2: LinkedIn connection request
- Day 3: Follow-up call (morning)
- Day 5: Second email with value-add content
- Day 7: Call attempt #2
- Day 10: Final email with specific ask

### BD Recommendations
- Immediate outreach recommended - Hot opportunity
- Escalate to BD leadership for review
- High-value cleared position - prioritize
- San Diego/PACAF - critical understaffed site

---

## Source Information

**Job URL:** https://example.com/job/sample-001
**Date Posted:** 2025-01-10
**Scraped:** N/A

---

*Generated by BD Automation Engine - Engine4_Playbook v1.0*
